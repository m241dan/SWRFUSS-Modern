...
char* sprint_reset(RESET_DATA* pReset, short* num)
{
    ...
    switch (pReset->command)
    {
        default:
            snprintf(
                buf, MAX_STRING_LENGTH, "%2d) *** BAD RESET: %c %d %d %d %d ***\r\n",
                *num, pReset->command, pReset->extra, pReset->arg1, pReset->arg2, pReset->arg3
            );
            break;

        case 'M':mob = get_mob_index(pReset->arg1);
            ...
            snprintf(
                buf, MAX_STRING_LENGTH, "%2d) %s (%d) -> %s Room: %d [%d]\r\n", *num, mobname, pReset->arg1,
                ...
                switch (tReset->command)
                {
                    ...
                    case 'G':
                        ...
                        if (!(obj = get_obj_index(tReset->arg1)))
                            ...
                            mudstrlcpy(objname, obj->name, MAX_INPUT_LENGTH);
                        snprintf(
                            buf + strlen(buf), MAX_STRING_LENGTH - strlen(buf), "%2d) (carry) %s (%d) -> %s [%d]\r\n",
        ...
        case 'O':
            ...
            if (!room)
                ...
                mudstrlcpy(roomname, room->name, MAX_INPUT_LENGTH);
            snprintf(
                buf, MAX_STRING_LENGTH, "%2d) (object) %s (%d) -> %s Room: %d [%d]\r\n",
        ...
        case 'D':
            ...
            switch (pReset->arg3)
            {
                default:mudstrlcpy(mobname, "(* ERROR *)", MAX_INPUT_LENGTH);
                    break;
                case 0:mudstrlcpy(mobname, "Open", MAX_INPUT_LENGTH);
                    break;
                case 1:mudstrlcpy(mobname, "Close", MAX_INPUT_LENGTH);
                    break;
                case 2:mudstrlcpy(mobname, "Close and lock", MAX_INPUT_LENGTH);
                    break;
            }
            snprintf(
                buf, MAX_STRING_LENGTH, "%2d) %s [%d] the %s [%d] door %s (%d)\r\n",
        ...
        case 'T':
            if (!(room = get_room_index(pReset->arg3)))
                ...
                mudstrlcpy(roomname, room->name, MAX_INPUT_LENGTH);
            snprintf(
                buf, MAX_STRING_LENGTH, "%2d) Trap: %d %d %d %d (%s) -> %s (%d)\r\n",
...
int generate_itemlevel(AREA_DATA* pArea, OBJ_INDEX_DATA* pObjIndex)
{
    ...
    if (pObjIndex->level > 0)
        ...
        switch (pObjIndex->item_type)
        {
            default:olevel = 0;
                break;
            case ITEM_PILL:olevel = number_range(min, max);
                break;
            case ITEM_POTION:olevel = number_range(min, max);
                break;
            case ITEM_SCROLL:olevel = pObjIndex->value[0];
                break;
            case ITEM_WAND:olevel = number_range(min + 4, max + 1);
                break;
            case ITEM_STAFF:olevel = number_range(min + 9, max + 5);
                break;
            case ITEM_ARMOR:olevel = number_range(min + 4, max + 1);
                break;
            case ITEM_WEAPON:olevel = number_range(min + 4, max + 1);
                break;
        }
    return olevel;
}
...
void reset_room(ROOM_INDEX_DATA* room)
{
    ...
    for (pReset = room->first_reset; pReset; pReset = pReset->next)
    {
        ...
        switch (pReset->command)
        {
            default:bug("%s: %s: bad command %c.", __func__, filename, pReset->command);
                break;
                        ...
                        switch (tReset->command)
                        {
                            ...
                            case 'E':
                                ...
                                if (tReset->command == 'E')
                                {
                                    ...
                                    {
                                        bug(
                                            "%s: 'E' reset: can't give object %d to mob %d.",
                                            __func__,
                                            obj->pIndexData->vnum,
                                            mob->pIndexData->vnum
                                        );
                                    ...
                                    for (gReset = tReset->first_reset; gReset; gReset = gReset->next_reset)
                                    {
                                        ...
                                        switch (gReset->command)
                                        {
                                            ...
                                            case 'P':
                                                ...
                                                {
                                                    bug(
                                                        "%s: %s: 'P': Exceeded nesting limit of %d",
                                                        __func__,
                                                        filename,
                                                        MAX_NEST
                                                    );
                                                ...
                                                obj        = create_object(
                                                    pObjIndex,
                                                    number_fuzzy(UMAX(generate_itemlevel(
                                                        room->area,
                                                        pObjIndex
                                                    ), to_obj->level)));
                                                if (num > 1)
                    ...
                    for (tReset = pReset->first_reset; tReset; tReset = tReset->next_reset)
                    {
                        ...
                        switch (tReset->command)
                        {
                            ...
                            case 'P':
                                ...
                                {
                                    bug(
                                        "%s: %s: 'P': Exceeded nesting limit of %d. Room %d.",
                                        __func__,
                                        filename,
                                        MAX_NEST,
                                        room->vnum
                                ...
                                obj        = create_object(
                                    pObjIndex,
                                    number_fuzzy(UMAX(generate_itemlevel(room->area, pObjIndex),
                                                      to_obj->level)));
                                if (num > 1)
                                    pObjIndex->count += (num - 1);
            ...
            case 'D':
                ...
                switch (pReset->arg3)
                {
                    case 0:REMOVE_BIT(pexit->exit_info, EX_CLOSED);
...
RESET_DATA* add_reset(ROOM_INDEX_DATA* room, char letter, int extra, int arg1, int arg2, int arg3)
{
    ...
    switch (letter)
    {
        case 'M':room->last_mob_reset = pReset;
            break;
        ...
        case 'O':room->last_obj_reset = pReset;
            break;
