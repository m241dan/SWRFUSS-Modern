...
bool spec_newbie_pilot(CHAR_DATA* ch)
{
    ...
    for (auto* victim : ch->in_room->persons)
    {
        ...
        switch (victim->race)
        {
            ...
            default:snprintf(buf, MAX_STRING_LENGTH, "Hmm, a %s.", race_table[victim->race].race_name);
        ...
        for (vip = 0; vip < 32; vip++)
            if (IS_SET(ch->vip_flags, 1 << vip) && IS_SET(victim->pcdata->wanted_flags, 1 << vip))
            {
                ...
                if (ch->top_level >= victim->top_level)
                    ...
                    {
                        case 1:jail = get_room_index(ROOM_JAIL_MON_CALAMARI_1);
                            break;
                        case 2:jail = get_room_index(ROOM_JAIL_MON_CALAMARI_2);
                            break;
                        case 3:jail = get_room_index(ROOM_JAIL_QUARREN_2);
                            break;
                        case 4:jail = get_room_index(ROOM_JAIL_QUARREN_1);
                            break;
                    }
                else
                {
            ...
            if (IS_SET(ch->vip_flags, 1 << vip) && IS_SET(victim->pcdata->wanted_flags, 1 << vip))
            {
                ...
                if (1 << vip == VIP_ADARI)
                    ...
                    switch (number_range(1, 4))
                    {
                        ...
                        case 2:jail = get_room_index(ROOM_JAIL_MON_CALAMARI_2);
                            break;
                        case 3:jail = get_room_index(ROOM_JAIL_QUARREN_1);
                            break;
                        case 4:jail = get_room_index(ROOM_JAIL_QUARREN_2);
                            break;
                    }
