...
void do_slookup(CHAR_DATA* ch, const char* argument)
{
    ...
    if (!str_cmp(arg, "all"))
    {
        for (sn = 0; sn < top_sn && skill_table[sn] && skill_table[sn]->name; sn++)
            pager_printf(
                ch, "Sn: %4d Slot: %4d Skill/spell: '%-20s' Damtype: %s\r\n",
    ...
    else
    {
        ...
        if (skill->flags)
        {
            ...
            ch_printf(
                ch, "Damtype: %s  Acttype: %s   Classtype: %s   Powertype: %s\r\n",
...
void do_sset(CHAR_DATA* ch, const char* argument)
{
        ...
        && !str_cmp(arg1, "create") && (!str_cmp(arg2, "skill") || !str_cmp(arg2, "herb")))
    {
        ...
        if (!str_cmp(arg2, "herb"))
        {
            ...
            {
                ch_printf(
                    ch, "The current top herb is %d, which is the maximum.  "
        ...
        {
            ch_printf(
                ch, "The current top sn is %d, which is the maximum.  "
        ...
        default:
            ...
            else
            {
                switch (ch->in_room->sector_type)
                {
                    ...
                    case SECT_UNDERWATER:send_to_char("You cannot dig here.\r\n", ch);
                        return;
...
void do_scan(CHAR_DATA* ch, const char* argument)
{
    ...
    for (dist = 1; dist <= max_dist;)
    {
        ...
        switch (ch->in_room->sector_type)
        {
            ...
            case SECT_INSIDE:
            case SECT_FIELD:
            case SECT_UNDERGROUND:dist++;
                break;
            case SECT_FOREST:
            case SECT_CITY:
            case SECT_DESERT:
            case SECT_HILLS:dist += 2;
                break;
            case SECT_WATER_SWIM:
            case SECT_WATER_NOSWIM:dist += 3;
                break;
            case SECT_MOUNTAIN:
            case SECT_UNDERWATER:
            case SECT_OCEANFLOOR:dist += 4;
                break;
        }
