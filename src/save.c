...
void de_equip_char(CHAR_DATA* ch)
{
    ...
    for (obj = ch->first_carrying; obj; obj = obj->next_content)
        if (obj->wear_loc > -1 && obj->wear_loc < MAX_WEAR)
        {
            ...
            {
                bug(
                    "%s: %s had on more than %d layers of clothing in one location (%d): %s", __func__,
...
bool load_char_obj(DESCRIPTOR_DATA* d, char* name, bool preload, bool copyover)
{
    ...
    if (stat(strsave, &fst) != -1)
    {
        ...
        {
            snprintf(
                buf, MAX_INPUT_LENGTH, "%s player data for: %s (%dK)",
    ...
    */
    ...
    for (;;)
    {
        ...
        switch (UPPER(word[0]))
        {
            ...
            case 'A':KEY("Act", ch->act, fread_number(fp));
            ...
            case 'B':KEY("Bamfin", ch->pcdata->bamfin, fread_string_nohash(fp));
            ...
            case 'G':KEY("Glory", ch->pcdata->quest_curr, fread_number(fp));
            ...
            case 'I':KEY("IllegalPK", ch->pcdata->illegal_pk, fread_number(fp));
            ...
            case 'P':KEY("Pagerlen", ch->pcdata->pagerlen, fread_number(fp));
            ...
            case 'R':KEY("Race", ch->race, fread_number(fp));
            ...
            case 'V':KEY("Version", file_ver, fread_number(fp));
                break;
...
void fread_obj(CHAR_DATA* ch, FILE* fp, short os_type)
{
    ...
    for (;;)
    {
        ...
        switch (UPPER(word[0]))
        {
            ...
            case 'C':KEY("Cost", obj->cost, fread_number(fp));
                KEY("Count", obj->count, fread_number(fp));
                break;

            case 'D':KEY("Description", obj->description, fread_string(fp));
                break;
            ...
            case 'I':KEY("ItemType", obj->item_type, fread_number(fp));
                break;

            case 'L':KEY("Level", obj->level, fread_number(fp));
                break;
            ...
            case 'R':KEY("Rvnum", obj->room_vnum, fread_number(fp));
            ...
            case 'T':KEY("Timer", obj->timer, fread_number(fp));
                break;
