    ...
    for (ch = last_char; ch; lst_ch = ch, ch = gch_prev)
    {
       ...
       */
        if (!ch->in_room || !ch->name)
        {
            ...
            snprintf(
                buf, MAX_STRING_LENGTH, "ch: %ld  ch->in_room: %ld  ch->prev: %ld  ch->next: %ld",
            ...
            if (lst_ch)
                snprintf(
                    buf, MAX_STRING_LENGTH, "lst_ch: %ld  lst_ch->prev: %ld  lst_ch->next: %ld",
    ...
    *gsn_ptr = -1;
    if (!IS_NPC(ch) && wield)
    {
        switch (wield->value[3])
        {
            default:*gsn_ptr = -1;
                break;
            case 3:*gsn_ptr = gsn_lightsabers;
                break;
            case 2:*gsn_ptr = gsn_vibro_blades;
                break;
            case 4:*gsn_ptr = gsn_flexible_arms;
                break;
            case 5:*gsn_ptr = gsn_talonous_arms;
                break;
            case 6:*gsn_ptr = gsn_blasters;
                break;
            case 8:*gsn_ptr = gsn_bludgeons;
                break;
            case 9:*gsn_ptr = gsn_bowcasters;
                break;
            case 11:*gsn_ptr = gsn_force_pikes;
                break;
...
ch_ret one_hit(CHAR_DATA* ch, CHAR_DATA* victim, int dt)
{
    ...
    */
        ...
        && dt == TYPE_UNDEFINED && IS_NPC(ch) && ch->attacks != 0)
    {
        ...
        {
            default:break;
            case ATCK_BITE:do_bite(ch, "");
                retcode = global_retcode;
                break;
            case ATCK_CLAWS:do_claw(ch, "");
                retcode = global_retcode;
                break;
            case ATCK_TAIL:do_tail(ch, "");
                retcode = global_retcode;
                break;
            case ATCK_STING:do_sting(ch, "");
                retcode = global_retcode;
                break;
            case ATCK_PUNCH:do_punch(ch, "");
                retcode = global_retcode;
                break;
            case ATCK_KICK:do_kick(ch, "");
                retcode = global_retcode;
                break;
            case ATCK_TRIP:attacktype = 0;
                break;
        }
        if (attacktype)
            return retcode;
    ...
    */
        ...
        else if (wield->blaster_setting == BLASTER_STUN && wield->value[4] >= 5)
        {
          ...
          */
            ...
            if (!fail && number_percent() < schance)
            {
                ...
                act(
                    AT_BLUE, "Blue rings of energy from your blaster strike $N, leaving $M stunned!", ch, nullptr, victim,
                    TO_CHAR
                );
                act(
                    AT_BLUE, "Blue rings of energy from $n's blaster hit $N, leaving $M stunned!", ch, nullptr, victim,
                    TO_NOTVICT
                );
            ...
            {
                act(
                    AT_BLUE, "Blue rings of energy from $N's blaster hit you but have little effect", victim, nullptr, ch,
                    TO_CHAR
                );
                act(
                    AT_BLUE,
    ...
    else if (dt == (TYPE_HIT + WEAPON_LIGHTSABER) && wield && wield->item_type == ITEM_WEAPON)
    {
        if (wield->value[4] < 1)
        {
            ...
            act(
                AT_YELLOW, "You need to recharge your lightsaber ... it seems to be lacking a blade.", ch, nullptr, victim,
                TO_CHAR
            );
            if (IS_NPC(ch))
            {
    ...
    */
    if (IS_NPC(victim))
    {
        ...
        if (owield != nullptr && owield->value[3] == WEAPON_BLASTER && get_cover(victim) == TRUE)
        {
            start_hating(victim, ch);
            start_hunting(victim, ch);
        }
...
ch_ret damage(CHAR_DATA* ch, CHAR_DATA* victim, int dam, int dt)
{
    ...
    */
    if (victim->position == POS_DEAD)
    {
        ...
        {
            snprintf(
                log_buf, MAX_STRING_LENGTH, "%s killed by %s at %d",
...
void do_flee(CHAR_DATA* ch, const char* argument)
{
    ...
    */
    ...
    for (attempt = 0; attempt < 8; attempt++)
    {
        ...
        door       = number_door();
        if ((pexit = get_exit(was_in, door)) == nullptr
            || !pexit->to_room
...
void do_slay(CHAR_DATA* ch, const char* argument)
{
    ...
    else if (!str_cmp(arg2, "shatter"))
    {
        ...
        act(
            AT_LBLUE, "$n freezes you with a glance and shatters your frozen body into tiny shards.", ch, nullptr, victim,
            TO_VICT
        );
        act(
            AT_LBLUE, "$n freezes $N with a glance and shatters the frozen body into tiny shards.", ch, nullptr, victim,
            TO_NOTVICT
        );
    }
    ...
    else if (!str_cmp(arg2, "demon"))
    {
        ...
        act(
            AT_IMMORT, "foul creature turns on $N, who screams in panic before being eaten alive.", ch, nullptr, victim,
            TO_CHAR
        );
        act(
            AT_IMMORT,
        ...
        act(
            AT_IMMORT, "you with a horrible grin.   You scream in panic before being eaten alive.", ch, nullptr, victim,
            TO_VICT
        );
        act(
            AT_IMMORT, "$n gestures, and a slavering demon appears.  With a horrible grin, the", ch, nullptr, victim,
            TO_NOTVICT
        );
        act(
            AT_IMMORT, "foul creature turns on $N, who screams in panic before being eaten alive.", ch, nullptr, victim,
