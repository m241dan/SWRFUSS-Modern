...
void affect_modify(CHAR_DATA* ch, AFFECT_DATA* paf, bool fAdd)
{
    ...
    else
    {
       ...
       */
        ...
        {
            case APPLY_AFFECT:REMOVE_BIT(ch->affected_by, mod);
                return;
            case APPLY_RESISTANT:REMOVE_BIT(ch->resistant, mod);
                return;
            case APPLY_IMMUNE:REMOVE_BIT(ch->immune, mod);
                return;
            case APPLY_SUSCEPTIBLE:REMOVE_BIT(ch->susceptible, mod);
                return;
            case APPLY_REMOVE:SET_BIT(ch->affected_by, mod);
                return;
        }
        mod = 0 - mod;
    }
    ...
    switch (paf->location % REVERSE_APPLY)
    {
        default:bug("%s: unknown location %d.", __func__, paf->location);
            return;

        case APPLY_NONE:break;
        case APPLY_STR:ch->mod_str += mod;
            break;
        case APPLY_DEX:ch->mod_dex += mod;
            break;
        case APPLY_INT:ch->mod_int += mod;
            break;
        case APPLY_WIS:ch->mod_wis += mod;
            break;
        case APPLY_CON:ch->mod_con += mod;
            break;
        case APPLY_CHA:ch->mod_cha += mod;
            break;
        case APPLY_LCK:ch->mod_lck += mod;
            break;
        case APPLY_SEX:ch->sex = (ch->sex + mod) % 3;
            if (ch->sex < 0)
                ch->sex += 2;
        ...
        case APPLY_HEIGHT:ch->height += mod;
            break;
        case APPLY_WEIGHT:ch->weight += mod;
            break;
        ...
        case APPLY_AC:ch->armor += mod;
            break;
        case APPLY_HITROLL:ch->hitroll += mod;
            break;
        case APPLY_DAMROLL:ch->damroll += mod;
            break;
        case APPLY_SAVING_POISON:ch->saving_poison_death += mod;
            break;
        case APPLY_SAVING_ROD:ch->saving_wand += mod;
            break;
        case APPLY_SAVING_PARA:ch->saving_para_petri += mod;
            break;
        case APPLY_SAVING_BREATH:ch->saving_breath += mod;
            break;
        case APPLY_SAVING_SPELL:ch->saving_spell_staff += mod;
            break;
        case APPLY_AFFECT:SET_BIT(ch->affected_by, mod);
            break;
        case APPLY_RESISTANT:SET_BIT(ch->resistant, mod);
            break;
        case APPLY_IMMUNE:SET_BIT(ch->immune, mod);
            break;
        case APPLY_SUSCEPTIBLE:SET_BIT(ch->susceptible, mod);
            break;
        case APPLY_WEAPONSPELL:   /* see fight.c */
            break;
        case APPLY_REMOVE:REMOVE_BIT(ch->affected_by, mod);
            break;
        ...
        case APPLY_MENTALSTATE:ch->mental_state = URANGE(-100, ch->mental_state + mod, 100);
            break;
        case APPLY_EMOTION:ch->emotional_state = URANGE(-100, ch->emotional_state + mod, 100);
            break;
    ...
    */
    if (!IS_NPC(ch)
        && saving_char != ch
        && (wield = get_eq_char(ch, WEAR_WIELD)) != nullptr && get_obj_weight(wield) > str_app[get_curr_str(ch)].wield)
    {
        static int depth;
...
void char_from_room(CHAR_DATA* ch)
{
    ...
    if (!ch)
    {
        bug("%s: nullptr char.", __func__);
        return;
    }

    if (!ch->in_room)
    {
        bug("%s: nullptr room: %s", __func__, ch->name);
        return;
    }
...
void char_to_room(CHAR_DATA* ch, ROOM_INDEX_DATA* pRoomIndex)
{
    ...
    {
        bug("%s: %s -> nullptr room!  Putting char in limbo (%d)", __func__, ch->name, ROOM_VNUM_LIMBO);
        /*
    ...
    for (RQueue = first_relation; RQueue; RQueue = rq_next)
    {
        ...
        if (RQueue->Type == relOSET_ON)
        {
            if (obj == RQueue->Subject)
                ((CHAR_DATA*)RQueue->Actor)->dest_buf = nullptr;
            else
                continue;
...
OBJ_DATA* find_obj(CHAR_DATA* ch, const char* argument, bool carryonly)
{
    ...
    else
    {
        ...
        if (carryonly
            && (container = get_obj_carry(ch, arg2)) == nullptr && (container = get_obj_wear(ch, arg2)) == nullptr)
        {
            send_to_char("You do not have that item.\r\n", ch);
            return nullptr;
        }
        ...
        if (!obj)
            act(
                ...
                "I see nothing like that beneath $p." : "I see nothing like that in $p.",
                ch,
                container,
                nullptr,
                TO_CHAR
...
ch_ret spring_trap(CHAR_DATA* ch, OBJ_DATA* obj)
{
    ...
    switch (typ)
    {
        default:txt = "hit by a trap";
            break;
        case TRAP_TYPE_POISON_GAS:txt = "surrounded by a green cloud of gas";
            break;
        case TRAP_TYPE_POISON_DART:txt = "hit by a dart";
            break;
        case TRAP_TYPE_POISON_NEEDLE:txt = "pricked by a needle";
            break;
        case TRAP_TYPE_POISON_DAGGER:txt = "stabbed by a dagger";
            break;
        case TRAP_TYPE_POISON_ARROW:txt = "struck with an arrow";
            break;
        case TRAP_TYPE_BLINDNESS_GAS:txt = "surrounded by a red cloud of gas";
            break;
        case TRAP_TYPE_SLEEPING_GAS:txt = "surrounded by a yellow cloud of gas";
            break;
        case TRAP_TYPE_FLAME:txt = "struck by a burst of flame";
            break;
        case TRAP_TYPE_EXPLOSION:txt = "hit by an explosion";
            break;
        case TRAP_TYPE_ACID_SPRAY:txt = "covered by a spray of acid";
            break;
        case TRAP_TYPE_ELECTRIC_SHOCK:txt = "suddenly shocked";
            break;
        case TRAP_TYPE_BLADE:txt = "sliced by a razor sharp blade";
            break;
        case TRAP_TYPE_SEX_CHANGE:txt = "surrounded by a mysterious aura";
            break;
    }
    ...
    switch (typ)
    {
        ...
        case TRAP_TYPE_POISON_ARROW:
          ...
          */
            ...
            break;
        case TRAP_TYPE_POISON_GAS:retcode = obj_cast_spell(gsn_poison, lev, ch, ch, nullptr);
            break;
        case TRAP_TYPE_BLINDNESS_GAS:retcode = obj_cast_spell(gsn_blindness, lev, ch, ch, nullptr);
            break;
        case TRAP_TYPE_SLEEPING_GAS:retcode = obj_cast_spell(skill_lookup("sleep"), lev, ch, ch, nullptr);
            break;
        case TRAP_TYPE_ACID_SPRAY:retcode = obj_cast_spell(skill_lookup("acid blast"), lev, ch, ch, nullptr);
            break;
        case TRAP_TYPE_SEX_CHANGE:retcode = obj_cast_spell(skill_lookup("change sex"), lev, ch, ch, nullptr);
            break;
        case TRAP_TYPE_FLAME:
        case TRAP_TYPE_EXPLOSION:retcode = obj_cast_spell(gsn_fireball, lev, ch, ch, nullptr);
            break;
        case TRAP_TYPE_ELECTRIC_SHOCK:
        case TRAP_TYPE_BLADE:retcode = damage(ch, ch, dam, TYPE_UNDEFINED);
    }
    return retcode;
}
    ...
    if (paf->location != APPLY_NONE && paf->modifier != 0)
    {
        switch (paf->location)
        {
            default:
                ...
                break;
            case APPLY_AFFECT:snprintf(buf, MAX_STRING_LENGTH, "Affects %s by", affect_loc_name(paf->location));
                for (x = 0; x < 32; x++)
            ...
            case APPLY_REMOVESPELL:
                snprintf(
                    buf, MAX_STRING_LENGTH, "Casts spell '%s'\r\n",
                    IS_VALID_SN(paf->modifier) ? skill_table[paf->modifier]->name : "unknown"
                );
                break;
            case APPLY_RESISTANT:
            case APPLY_IMMUNE:
            case APPLY_SUSCEPTIBLE:snprintf(buf, MAX_STRING_LENGTH, "Affects %s by", affect_loc_name(paf->location));
                for (x = 0; x < 32; x++)
                    if (IS_SET(paf->modifier, 1 << x))
                    {
                        mudstrlcat(buf, " ", MAX_STRING_LENGTH);
