...
void do_track(CHAR_DATA* ch, const char* argument)
{
    ...
    switch (dir)
    {
        case BFS_ERROR:send_to_char("Hmm... something seems to be wrong.\r\n", ch);
            break;
        case BFS_ALREADY_THERE:send_to_char("You're already in the same room!\r\n", ch);
            break;
        case BFS_NO_PATH:send_to_char("You can't sense a trail from here.\r\n", ch);
            learn_from_failure(ch, gsn_track);
            break;
        default:ch_printf(ch, "You sense a trail %s from here...\r\n", dir_name[dir]);
...
void found_prey(CHAR_DATA* ch, CHAR_DATA* victim)
{
    ...
    if (IS_SET(ch->in_room->room_flags, ROOM_SAFE))
    {
        ...
        switch (number_bits(2))
        {
            case 0:do_say(ch, "C'mon out, you coward!");
                ...
                do_yell(ch, buf);
                break;
            case 1:snprintf(buf, MAX_STRING_LENGTH, "Let's take this outside, %s", victname);
                do_say(ch, buf);
                break;
    ...
    switch (number_bits(2))
    {
        case 0:snprintf(buf, MAX_STRING_LENGTH, "Your blood is mine, %s!", victname);
            do_yell(ch, buf);
            break;
        case 1:snprintf(buf, MAX_STRING_LENGTH, "Alas, we meet again, %s!", victname);
            do_say(ch, buf);
            break;
...
void hunt_victim(CHAR_DATA* ch)
{
    ...
    else
    {
        ...
        if (!ch->hunting)
        {
            ...
            {
                bug(
                    "%s: no ch->in_room!  Mob #%d, name: %s.  Placing mob in limbo.",
                    __func__,
                    ch->pIndexData->vnum,
                    ch->name
                );
