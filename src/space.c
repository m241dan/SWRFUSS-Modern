...
void update_traffic()
{
    ...
    if (senate != nullptr && shuttle != nullptr)
    {
        switch (corus_shuttle)
        {
            default:corus_shuttle++;
                break;

            case 0:land_bus(shuttle, STOP_PLANET);
            ...
            case 4:launch_bus(shuttle);
                launch_bus(senate);
                corus_shuttle++;
                break;

            case 5:land_bus(shuttle, STOP_SHIPYARD);
    ...
    if (turbocar != nullptr)
    {
        echo_to_room(
            AT_YELLOW,
            get_room_index(turbocar->location),
            "The turbocar doors close and it speeds out of the station."
        );
        ...
        echo_to_room(
            AT_YELLOW,
            get_room_index(turbocar->location),
            "A turbocar pulls into the platform and the doors slide open."
        );
...
void update_bus()
{
    ...
    if (ship == nullptr && ship2 == nullptr)
        return;

    switch (bus_pos)
    {
        case 0:target = ship_from_hanger(bus_vnum[bus_planet]);
            if (target != nullptr && !target->starsystem)
            {
                snprintf(
                    buf,
                    MAX_STRING_LENGTH,
                    "An electronic voice says, 'Cannot land at %s ... it seems to have dissapeared.'",
                    bus_stop[bus_planet]
                );
        ...
        case 6:launch_bus(ship);
            launch_bus(ship2);
            bus_pos++;
            break;

        case 7:echo_to_ship(AT_YELLOW, ship, "The ship lurches slightly as it makes the jump to lightspeed.");
        ...
        case 1:destination = bus_vnum[bus_planet];
            if (!land_bus(ship, destination))
            {
                snprintf(
                    buf,
                    MAX_STRING_LENGTH,
                    "An electronic voice says, 'Oh My, %s seems to have dissapeared.'",
                    bus_stop[bus_planet]
                );
                echo_to_ship(AT_CYAN, ship, buf);
                echo_to_ship(
        ...
        case 5:snprintf(buf, MAX_STRING_LENGTH, "It continues, 'Next stop, %s'", bus_stop[bus_planet + 1]);
    ...
    for (missile = first_missile; missile; missile = m_next)
    {
        ...
        if (target->starsystem && target->starsystem == missile->starsystem)
        {
                ...
                && abs((int)missile->mz) - abs((int)target->vz) >= -20)
            {
                ...
                {
                    echo_to_room(
                        AT_YELLOW, get_room_index(ship->gunseat),
    ...
    for (ship = first_ship; ship; ship = ship->next)
    {
       ...
       */
        ...
        if (ship->starsystem)
        {
            ...
            {
                snprintf(
                    buf, MAX_STRING_LENGTH, "Proximity alert: %s  %d %d %d", ship->starsystem->star1,
            ...
            {
                snprintf(
                    buf, MAX_STRING_LENGTH, "Proximity alert: %s  %d %d %d", ship->starsystem->star2,
            ...
            {
                snprintf(
                    buf, MAX_STRING_LENGTH, "Proximity alert: %s  %d %d %d", ship->starsystem->planet1,
            ...
            {
                snprintf(
                    buf, MAX_STRING_LENGTH, "Proximity alert: %s  %d %d %d", ship->starsystem->planet2,
            ...
            {
                snprintf(
                    buf, MAX_STRING_LENGTH, "Proximity alert: %s  %d %d %d", ship->starsystem->planet3,
...
void fread_starsystem(SPACE_DATA* starsystem, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (UPPER(word[0]))
        {
            ...
            case 'D':KEY("Doc1a", starsystem->doc1a, fread_number(fp));
            ...
            case 'F':KEY("Filename", starsystem->filename, fread_string_nohash(fp));
                break;

            case 'G':KEY("Gravitys1", starsystem->gravitys1, fread_number(fp));
            ...
            case 'L':KEY("Location1a", starsystem->location1a, fread_string(fp));
            ...
            case 'N':KEY("Name", starsystem->name, fread_string(fp));
                break;

            case 'P':KEY("Planet1", starsystem->planet1, fread_string(fp));
            ...
            case 'S':KEY("Star1", starsystem->star1, fread_string(fp));
            ...
            case 'X':KEY("Xpos", starsystem->xpos, fread_number(fp));

            case 'Y':KEY("Ypos", starsystem->ypos, fread_number(fp));
...
void showstarsystem(CHAR_DATA* ch, SPACE_DATA* starsystem)
{
    ...
    ch_printf(
        ch, "Planet2: %s   Gravity: %d   Coordinates: %d %d %d\r\n",
    ...
    ch_printf(
        ch, "Planet3: %s   Gravity: %d   Coordinates: %d %d %d\r\n",
...
void echo_to_system(
    int color, SHIP_DATA* ship, const char* argument,
    SHIP_DATA* ignore
)
{
    SHIP_DATA* target;
...
void fread_ship(SHIP_DATA* ship, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (UPPER(word[0]))
        {
            ...
            case 'A':KEY("Astro_array", ship->astro_array, fread_number(fp));
                break;

            case 'C':KEY("Cockpit", ship->cockpit, fread_number(fp));
            ...
            case 'D':KEY("Description", ship->description, fread_string(fp));
                break;
            ...
            case 'N':KEY("Name", ship->name, fread_string(fp));
                KEY("Navseat", ship->navseat, fread_number(fp));
                break;

            case 'O':KEY("Owner", ship->owner, fread_string(fp));
                KEY("Objectnum", dummy_number, fread_number(fp));
                break;

            case 'P':KEY("Pilot", ship->pilot, fread_string(fp));
                KEY("Pilotseat", ship->pilotseat, fread_number(fp));
                break;

            case 'R':KEY("Realspeed", ship->realspeed, fread_number(fp));
                KEY("Rockets", ship->rockets, fread_number(fp));
                break;

            case 'S':KEY("Shipyard", ship->shipyard, fread_number(fp));
            ...
            case 'T':KEY("Type", ship->type, fread_number(fp));
...
bool load_ship_file(const char* shipfile)
{
    ...
    else
    {
        ...
        else if ((pRoomIndex = get_room_index(ship->lastdoc)) != nullptr
                 && ship->ship_class != CAPITAL_SHIP
                 && ship->ship_class != SHIP_PLATFORM)
        {
...
void do_showship(CHAR_DATA* ch, const char* argument)
{
    ...
    ch_printf(
        ch, "Cockpit: %d   Entrance: %d   Hanger: %d  Engineroom: %d\r\n",
        ship->cockpit, ship->entrance, ship->hanger, ship->engineroom
    );
    ch_printf(
        ch, "Pilotseat: %d   Coseat: %d   Navseat: %d  Gunseat: %d\r\n",
    ...
    ch_printf(
        ch, "Tractor Beam: %d   Comm: %d   Sensor: %d   Astro Array: %d\r\n",
        ship->tractorbeam, ship->comm, ship->sensor, ship->astro_array
    );
    ch_printf(
        ch,
        "Lasers: %d  Laser Condition: %s\r\n",
        ship->lasers,
        ship->statet0 == LASER_DAMAGED ? "Damaged" : "Good"
    );
    ch_printf(
        ch,
        "Turret One: %d  Condition: %s\r\n",
        ship->turret1,
        ship->statet1 == LASER_DAMAGED ? "Damaged" : "Good"
    );
    ch_printf(
        ch,
        "Turret Two: %d  Condition: %s\r\n",
        ship->turret2,
        ship->statet2 == LASER_DAMAGED ? "Damaged" : "Good"
    );
    ch_printf(
        ch, "Missiles: %d/%d  Torpedos: %d/%d  Rockets: %d/%d  Condition: %s\r\n",
        ship->missiles,
        ship->maxmissiles,
        ship->torpedos,
        ship->maxtorpedos,
    ...
    ch_printf(
        ch, "Hull: %d/%d  Ship Condition: %s\r\n",
        ship->hull, ship->maxhull, ship->shipstate == SHIP_DISABLED ? "Disabled" : "Running"
    );

    ch_printf(
        ch, "Shields: %d/%d   Energy(fuel): %d/%d   Chaff: %d/%d\r\n",
...
void do_land(CHAR_DATA* ch, const char* argument)
{
    ...
    if (number_percent() < schance)
    {
        ...
        act(AT_PLAIN, "$n begins the landing sequence.", ch, nullptr, argument, TO_ROOM);
...
void do_info(CHAR_DATA* ch, const char* argument)
{
    ...
    act(AT_PLAIN, "$n checks various gages and displays on the control panel.", ch, nullptr, argument, TO_ROOM);
}
...
void do_fire(CHAR_DATA* ch, const char* argument)
{
    ...
    schance = IS_NPC(ch) ? ch->top_level
                         : (int)(
            ch->perm_dex * 2 + ch->pcdata->learned[gsn_spacecombat] / 3
                                                                                                                            ...
                                                                                                                            if( pexit )
   {
    ...
    if (!IS_IMMORTAL(ch) && !IS_NPC(ch) && ch->in_room->area != to_room->area)
    {
        if (ch->top_level < to_room->area->low_hard_range)
        {
            ...
            {
                case 1:send_to_char("A voice in your mind says, 'You are nearly ready to go that way...'", ch);
                    break;
                case 2:
                    send_to_char(
                ...
                case 3:
                    ...
                    break;
                default:send_to_char("A voice in your mind says, 'You are not ready to go down that path.'.\r\n", ch);
            }
            return rNONE;
        }
    ...
    switch (door)
    {
        default:dtxt = "somewhere";
            break;
        case 0:dtxt = "the south";
            break;
        case 1:dtxt = "the west";
            break;
        case 2:dtxt = "the north";
            break;
        case 3:dtxt = "the east";
            break;
        case 4:dtxt = "below";
            break;
        case 5:dtxt = "above";
            break;
        case 6:dtxt = "the south-west";
            break;
        case 7:dtxt = "the south-east";
            break;
        case 8:dtxt = "the north-west";
            break;
        case 9:dtxt = "the north-east";
            break;
    }
