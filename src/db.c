...
void load_objects(AREA_DATA* tarea, FILE* fp)
{
    ...
    for (;;)
    {
       ...
       */
        ...
        {
            case ITEM_PILL:
            case ITEM_POTION:pObjIndex->value[1] = slot_lookup(pObjIndex->value[1]);
                pObjIndex->value[2]              = slot_lookup(pObjIndex->value[2]);
                pObjIndex->value[3]              = slot_lookup(pObjIndex->value[3]);
                break;

            case ITEM_DEVICE:pObjIndex->value[3] = slot_lookup(pObjIndex->value[3]);
                break;
            case ITEM_SALVE:pObjIndex->value[4] = slot_lookup(pObjIndex->value[4]);
...
void load_resets(AREA_DATA* tarea, FILE* fp)
{
    ...
    for (;;)
    {
       ...
       */
        switch (letter)
        {
            default:bug("%s: bad command '%c'.", __func__, letter);
                if (fBootDb)
            ...
            case 'H':bug("%s: Unable to convert legacy hide reset. Must be converted manually.", __func__);
                break;
...
void load_specials(FILE* fp)
{
    for (;;)
    {
        ...
        {
            default:bug("%s: letter '%c' not *MS.", __func__, letter);
                exit(1);
...
OBJ_DATA* create_object(OBJ_INDEX_DATA* pObjIndex, int level)
{
    ...
    */
    ...
    {
        default:bug("%s: vnum %d bad type: %d.", __func__, pObjIndex->vnum, obj->item_type);
            break;
        ...
        case ITEM_SALVE:obj->value[3] = number_fuzzy(obj->value[3]);
            break;

        case ITEM_DEVICE:obj->value[0] = number_fuzzy(obj->value[0]);
        ...
        case ITEM_POTION:
        case ITEM_PILL:obj->value[0] = number_fuzzy(number_fuzzy(obj->value[0]));
            break;

        case ITEM_MONEY:obj->value[0] = obj->cost;
            break;
    }
...
void log_string_plus(const char* str, short log_type, short level)
{
    ...
    switch (log_type)
    {
        default:to_channel(str + offset, CHANNEL_LOG, "Log", level);
            break;
        case LOG_BUILD:to_channel(str + offset, CHANNEL_BUILD, "Build", level);
            break;
        case LOG_COMM:to_channel(str + offset, CHANNEL_COMM, "Comm", level);
            break;
        case LOG_ALL:break;
    }
}
    ...
    for (wiz = first_wiz; wiz; wiz = wiz->next)
    {
        if (wiz->level > LEVEL_AVATAR)
        {
            if (wiz->level < ilevel)
            {
                ...
                switch (ilevel)
                {
                    case MAX_LEVEL - 0:towizfile(" The President");
                        break;
                    case MAX_LEVEL - 1:towizfile(" The Executive Council");
                        break;
                    case MAX_LEVEL - 2:towizfile(" The Senate");
                        break;
                    case MAX_LEVEL - 4:towizfile(" The Retired");
                        break;
                    default:towizfile(" Galactic Engineers");
                        break;
                }
            }
...
void mobprog_file_read(MOB_INDEX_DATA* mob, const char* f)
{
    ...
    for (;;)
    {
        ...
        switch (mprg->type)
        {
            case ERROR_PROG:bug("%s: mudprog file type error", __func__);
                DISPOSE(mprg);
                continue;

            case IN_FILE_PROG:bug("%s: Nested file programs are not allowed.", __func__);
                DISPOSE(mprg);
                continue;

            default:mprg->arglist = fread_string(progfile);
...
void mprog_read_programs(FILE* fp, MOB_INDEX_DATA* mob)
{
    ...
    for (;;)
    {
        ...
        switch (mprg->type)
        {
            case ERROR_PROG:bug("%s: vnum %d MUDPROG type.", __func__, mob->vnum);
                exit(1);

            case IN_FILE_PROG:mprg->arglist = fread_string(fp);
                mprg->fileprog              = FALSE;
                mobprog_file_read(mob, mprg->arglist);
                break;

            default:SET_BIT(mob->progtypes, mprg->type);
                mprg->fileprog = FALSE;
...
void oprog_read_programs(FILE* fp, OBJ_INDEX_DATA* obj)
{
    ...
    for (;;)
    {
        ...
        switch (mprg->type)
        {
            case ERROR_PROG:bug("%s: vnum %d MUDPROG type.", __func__, obj->vnum);
                exit(1);
            ...
            default:SET_BIT(obj->progtypes, mprg->type);
                mprg->fileprog = FALSE;
                mprg->arglist  = fread_string(fp);
                mprg->comlist  = fread_string(fp);
                break;
...
void rprog_read_programs(FILE* fp, ROOM_INDEX_DATA* room)
{
    ...
    for (;;)
    {
        ...
        switch (mprg->type)
        {
            case ERROR_PROG:bug("%s: vnum %d MUDPROG type.", __func__, room->vnum);
                exit(1);
            ...
            default:SET_BIT(room->progtypes, mprg->type);
                mprg->fileprog = FALSE;
                mprg->arglist  = fread_string(fp);
                mprg->comlist  = fread_string(fp);
                break;
...
void process_sorting(AREA_DATA* tarea)
{
    ...
    fprintf(
        stderr, "%-14s: Rooms: %5d - %-5d Objs: %5d - %-5d Mobs: %5d - %d\n",
        tarea->filename,
...
EXTRA_DESCR_DATA* fread_fuss_exdesc(FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            default:log_printf("%s: no match: %s", __func__, word);
                fread_to_eol(fp);
                fMatch = TRUE;
                break;
            ...
            case 'E':KEY("ExDescKey", ed->keyword, fread_string(fp));
...
void fread_fuss_exit(FILE* fp, ROOM_INDEX_DATA* pRoomIndex)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            default:log_printf("%s: no match: %s", __FUNCTION__, word);
                fread_to_eol(fp);
                fMatch = TRUE;
                break;
            ...
            case 'K':KEY("Key", pexit->key, fread_number(fp));
                KEY("Keywords", pexit->keyword, fread_string(fp));
                break;

            case 'T':KEY("ToRoom", pexit->vnum, fread_number(fp));
                break;
        }
        ...
        if (!str_cmp(word, "MUDPROG"))
        {
            ...
            for (;;)
            {
                ...
                switch (word[0])
                {
                    ...
                    case 'C':KEY("Comlist", mprg->comlist, fread_string(progfile));
                        break;
        ...
        switch (word[0])
        {
            ...
            case 'A':
                if (!str_cmp(word, "Arglist"))
                {
                    ...
                    switch (mprg->type)
                    {
                        case IN_FILE_PROG:rprog_file_read(prog_target, mprg->arglist);
                            break;
                        default:break;
                    }
                    fMatch = TRUE;
            ...
            case 'C':KEY("Comlist", mprg->comlist, fread_string(fp));
                break;
...
void fread_fuss_room(FILE* fp, AREA_DATA* tarea)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            default:bug("%s: no match: %s", __func__, word);
                fread_to_eol(fp);
                fMatch = TRUE;
                break;
            ...
            case 'D':KEY("Desc", pRoomIndex->description, fread_string(fp));
                break;
            ...
            case 'N':KEY("Name", pRoomIndex->name, fread_string(fp));
                break;
        ...
        switch (word[0])
        {
            ...
            case 'A':
                if (!str_cmp(word, "Arglist"))
                {
                    ...
                    switch (mprg->type)
                    {
                        case IN_FILE_PROG:oprog_file_read(prog_target, mprg->arglist);
                            break;
                        default:break;
                    }
                    fMatch = TRUE;
...
void fread_fuss_object(FILE* fp, AREA_DATA* tarea)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            ...
            case 'K':KEY("Keywords", pObjIndex->name, fread_string(fp));
                break;

            case 'L':KEY("Long", pObjIndex->description, fread_string(fp));
                break;
            ...
            case 'S':KEY("Short", pObjIndex->short_descr, fread_string(fp));
                if (!str_cmp(word, "Spells"))
                {
                    switch (pObjIndex->item_type)
                    {
                        ...
                        case ITEM_PILL:
                        case ITEM_POTION:
                        case ITEM_SCROLL:pObjIndex->value[1] = skill_lookup(fread_word(fp));
                            pObjIndex->value[2]              = skill_lookup(fread_word(fp));
                            pObjIndex->value[3]              = skill_lookup(fread_word(fp));
                            break;

                        case ITEM_STAFF:
                        case ITEM_WAND:pObjIndex->value[3] = skill_lookup(fread_word(fp));
                            break;

                        case ITEM_SALVE:pObjIndex->value[4] = skill_lookup(fread_word(fp));
        ...
        switch (word[0])
        {
            ...
            case 'A':
                if (!str_cmp(word, "Arglist"))
                {
                    ...
                    switch (mprg->type)
                    {
                        case IN_FILE_PROG:mprog_file_read(prog_target, mprg->arglist);
                            break;
                        default:break;
                    }
                    fMatch = TRUE;
        ...
        switch (word[0])
        {
            ...
            case 'D':
                ...
                if (!str_cmp(word, "DefPos"))
                {
                    ...
                    {
                        bug(
                            "%s: vnum %d: Mobile in invalid default position! Defaulting to standing.",
                            __func__,
                            pMobIndex->vnum
                        );
            ...
            case 'K':KEY("Keywords", pMobIndex->player_name, fread_string(fp));
                break;

            case 'L':KEY("Long", pMobIndex->long_descr, fread_string(fp));
                break;
            ...
            case 'P':
                if (!str_cmp(word, "Position"))
                {
                    ...
                    {
                        bug(
                            "%s: vnum %d: Mobile in invalid position! Defaulting to standing.",
                            __func__,
                            pMobIndex->vnum
                        );
                        position = POS_STANDING;
...
void fread_fuss_areadata(FILE* fp, AREA_DATA* tarea)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            ...
            case 'A':KEY("Author", tarea->author, fread_string(fp));
                break;
            ...
            case 'N':KEY("Name", tarea->name, fread_string_nohash(fp));
                break;
            ...
            case 'V':KEY("Version", tarea->version, fread_number(fp));
                break;
        }
        if (!fMatch)
        {
    ...
    while (dentry)
    {
        if (dentry->d_name[0] != '.')
        {
            ...
            pArea->name = str_dup( buf );
                ...
                fprintf(
                    stderr, "%-14s: Rooms: %5d - %-5d Objs: %5d - %-5d "
                            "Mobs: %5d - %-5d\n",
                    pArea->filename,
 ...
 */
void show_vnums(
    CHAR_DATA* ch, int low, int high, bool proto, bool shownl,
    ...
    for (;;)
    {
        ...
        switch (UPPER(word[0]))
        {
            case '*':fMatch = TRUE;
                fread_to_eol(fp);
                break;
            case 'B':KEY("Build", sys->build_level, fread_number(fp));
                break;

            case 'D':KEY("Damplrvsplr", sys->dam_plr_vs_plr, fread_number(fp));
            ...
            case 'F':KEY("Forcepc", sys->level_forcepc, fread_number(fp));
                break;

            case 'G':KEY("Guildoverseer", sys->guild_overseer, fread_string_nohash(fp));
                KEY("Guildadvisor", sys->guild_advisor, fread_string_nohash(fp));
                break;

            case 'H':KEY("Highplayers", sys->alltimemax, fread_number(fp));
                KEY("Highplayertime", sys->time_of_max, fread_string_nohash(fp));
                break;

            case 'L':KEY("Log", sys->log_level, fread_number(fp));
                break;

            case 'M':KEY("Msetplayer", sys->level_mset_player, fread_number(fp));
                KEY("Muse", sys->muse_level, fread_number(fp));
                break;

            case 'N':KEY("Nameresolving", sys->NO_NAME_RESOLVING, fread_number(fp));
                break;

            case 'O':KEY("Overridepriv", sys->level_override_private, fread_number(fp));
                break;

            case 'P':KEY("Protoflag", sys->level_modify_proto, fread_number(fp));
                break;

            case 'R':KEY("Readallmail", sys->read_all_mail, fread_number(fp));
                KEY("Readmailfree", sys->read_mail_free, fread_number(fp));
                break;

            case 'S':KEY("Stunplrvsplr", sys->stun_plr_vs_plr, fread_number(fp));
            ...
            case 'T':KEY("Takeothersmail", sys->take_others_mail, fread_number(fp));
                KEY("Think", sys->think_level, fread_number(fp));
                break;


            case 'W':KEY("Waitforauth", sys->WAIT_FOR_AUTH, fread_number(fp));
