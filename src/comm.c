...
void nanny_confirm_new_name(DESCRIPTOR_DATA* d, const char* argument)
{
    ...
    switch (*argument)
    {
        ...
        case 'n':
        case 'N':write_to_buffer(d, "Ok, what IS it, then? ", 0);
            /*
       * clear descriptor pointer to get rid of bug message in log
       */
            d->character->desc = nullptr;
        ...
        default:write_to_buffer(d, "Please type Yes or No. ", 0);
            break;
    }
}
...
void nanny_get_new_sex(DESCRIPTOR_DATA* d, const char* argument)
{
    ...
    {
        case 'm':
        case 'M':ch->sex = SEX_MALE;
            break;
        case 'f':
        case 'F':ch->sex = SEX_FEMALE;
            break;
        case 'n':
        case 'N':ch->sex = SEX_NEUTRAL;
            break;
...
void nanny_get_want_ripansi(DESCRIPTOR_DATA* d, const char* argument)
{
    ...
    {
        case 'a':
        case 'A':SET_BIT(ch->act, PLR_ANSI);
            break;
        case 'n':
        case 'N':break;
        default:write_to_buffer(d, "Invalid selection.\r\nANSI or NONE? ", 0);
...
void nanny_get_msp(DESCRIPTOR_DATA* d, const char* argument)
{
    ...
    {
        case 'y':
        case 'Y':SET_BIT(ch->act, PLR_SOUND);
            break;
        case 'n':
        case 'N':break;
        default:write_to_buffer(d, "Invalid selection.\r\nYES or NO? ", 0);
...
char* act_string(const char* format, CHAR_DATA* to, CHAR_DATA* ch, const void* arg1, const void* arg2)
{
    ...
    while (*str != '\0')
    {
        ...
        else
        {
            switch (*str)
            {
                default:bug("%s: bad code %c.", __func__, *str);
                    i = " <@@@> ";
                    break;
                ...
                case 'q':i = (to == ch) ? "" : "s";
                    break;
                case 'Q':i = (to == ch) ? "your" : his_her[URANGE(0, ch->sex, 2)];
                    break;
...
void display_prompt(DESCRIPTOR_DATA* d)
{
    ...
    for (; *prompt; prompt++)
    {
       ...
       */
        ...
        switch (*(prompt - 1))
        {
            default:bug("%s: bad command char '%c'.", __func__, *(prompt - 1));
                break;
            case '%':*pbuf = '\0';
                pstat = 0x80000000;
                switch (*prompt)
                {
                    ...
                    case 'M':
                        if (IS_IMMORTAL(ch) || ch->skill_level[FORCE_ABILITY] > 1)
                            ...
                            pstat = 0;
                        break;
                    case 'u':pstat = num_descriptors;
                        break;
                    case 'U':pstat = sysdata.maxplayers;
                        break;
                    case 'v':pstat = ch->move;
                        break;
                    case 'V':pstat = ch->max_move;
                        break;
                    case 'g':pstat = ch->gold;
                        break;
                    case 'r':
                        if (IS_IMMORTAL(och))
