...
void get_obj(CHAR_DATA* ch, OBJ_DATA* obj, OBJ_DATA* container)
{
    ...
    {
        act(
            AT_ACTION, IS_OBJ_STAT(container, ITEM_COVERING) ?
          ...
          */
            ...
            if (!obj)
            {
                act(
                    ...
                    "I see nothing like that beneath the $T." : "I see nothing like that in the $T.",
                    ch,
                    nullptr,
            ...
            if (!found)
            {
                if (fAll)
                    act(
                        AT_PLAIN,
                        IS_OBJ_STAT(container, ITEM_COVERING) ?
                        "I see nothing beneath the $T." : "I see nothing in the $T.",
                        ch,
                        nullptr,
                        container->short_descr,
                        TO_CHAR
                    ...
                    act(
                        AT_PLAIN, IS_OBJ_STAT(container, ITEM_COVERING) ?
                                  "I see nothing like that beneath the $T." :
                                  "I see nothing like that in the $T.", ch, nullptr, container->short_descr, TO_CHAR
                    );
            }
            else
                check_for_trap(ch, container, TRAP_GET);
       ...
       */
        ...
        if (!str_cmp(arg, "credits") || !str_cmp(arg, "credit"))
        {
            ...
            for (obj = ch->in_room->first_content; obj; obj = obj_next)
            {
                ...
                {
                    case OBJ_VNUM_MONEY_ONE:number += 1;
                        extract_obj(obj);
                        break;

                    case OBJ_VNUM_MONEY_SOME:number += obj->value[0];
                        extract_obj(obj);
                        break;
                }
            }
...
void do_give(CHAR_DATA* ch, const char* argument)
{
    ...
    if (is_number(arg1))
    {
       ...
       */
        ...
        act(AT_ACTION, "You give $N some credits.", ch, nullptr, victim, TO_CHAR);
        send_to_char("OK.\r\n", ch);
...
void wear_obj(CHAR_DATA* ch, OBJ_DATA* obj, bool fReplace, short wear_bit)
{
    ...
    if (wear_bit > -1)
    {
        ...
        if (!CAN_WEAR(obj, 1 << bit))
        {
            if (fReplace)
            {
                switch (1 << bit)
                {
                    case ITEM_HOLD:send_to_char("You cannot hold that.\r\n", ch);
                        break;
                    case ITEM_WIELD:send_to_char("You cannot wield that.\r\n", ch);
                        break;
                    default:ch_printf(ch, "You cannot wear that on your %s.\r\n", w_flags[bit]);
                }
            }
            return;
        }
    ...
    */
    if (check_size)
    {
        if (ch->race == RACE_DUINUOGWUIN)
        {
            ...
            act(AT_ACTION, "$n tries to use $p, but is too big.", ch, obj, nullptr, TO_ROOM);
            return;
        }
    ...
    */
    ...
    switch (1 << bit)
    {
        default:bug("%s: unknown/unused item_wear bit %d", __func__, bit);
            if (fReplace)
                send_to_char("You can't wear, wield, or hold that.\r\n", ch);
            return;
...
*/
        ...
        case ITEM_HOLD:
            ...
            if (obj->item_type == ITEM_DEVICE
                ...
                || obj->item_type == ITEM_KEY || !oprog_use_trigger(ch, obj, nullptr, nullptr, nullptr))
            {
...
void do_bury(CHAR_DATA* ch, const char* argument)
{
    ...
    {
        case SECT_CITY:
        case SECT_INSIDE:send_to_char("The floor is too hard to dig through.\r\n", ch);
            return;
        case SECT_WATER_SWIM:
        case SECT_WATER_NOSWIM:
        case SECT_UNDERWATER:send_to_char("You cannot bury something here.\r\n", ch);
            return;
        case SECT_AIR:send_to_char("What?  In the air?!\r\n", ch);
            return;
    }
...
void do_auction(CHAR_DATA* ch, const char* argument)
{
    ...
    if (auction->item == nullptr)
    {
        switch (obj->item_type)
        {
            ...
            default:act(AT_TELL, "You cannot auction $Ts.", ch, nullptr, item_type_name(obj), TO_CHAR);
                return;
            ...
            case ITEM_BOOK:
            case ITEM_FABRIC:
            case ITEM_ARMOR:separate_obj(obj);
                obj_from_char(obj);
                if (IS_SET(sysdata.save_flags, SV_AUCTION))
                    save_char_obj(ch);
                auction->item     = obj;
