...
void do_goto(CHAR_DATA* ch, const char* argument)
{
    ...
    if (get_trust(ch) < LEVEL_IMMORTAL)
    {
        ...
        if ((
                ch->in_room->vnum < pArea->low_r_vnum
        ...
        if (IS_OBJ_STAT(obj, ITEM_PROTOTYPE))
        {
            ...
            for (paf  = pObjIndex->first_affect; paf; paf = paf->next)
            {
                if (++count == loc)
                {
                    ...
                    if (paf->location != APPLY_WEARSPELL &&
...
void do_redit(CHAR_DATA* ch, const char* argument)
{
    ...
    if (!compare(arg, "exit"))
    {
        ...
        switch (arg2[0])
        {
            default:edir = get_dir(arg2);
                break;
            case '+':edir = get_dir(arg2 + 1);
                addexit   = TRUE;
                break;
            case '#':edir = atoi(arg2 + 1);
                numnotdir = TRUE;
                break;
        }
        if (arg3[0] == '\0')
...
void do_astat(CHAR_DATA* ch, const char* argument)
{
    ...
    {
        ch_printf(
            ch, "Max players: %d  IllegalPks: %d  Credits Looted: %d\r\n",
        ...
        ch_printf(
            ch, "Mdeaths: %d  Mkills: %d  Pdeaths: %d  Pkills: %d\r\n",
    ...
    ch_printf(
        ch, "soft range: %d - %d.  hard range: %d - %d.\r\n",
        tarea->low_soft_range, tarea->hi_soft_range, tarea->low_hard_range, tarea->hi_hard_range
    );
...
void mpedit(
    CHAR_DATA* ch, MPROG_DATA* mprg, int mptype,
    const char* argument
)
{
    if (mptype != -1)
...
void do_mpedit(CHAR_DATA* ch, const char* argument)
{
    ...
    {
        default:break;
        case SUB_RESTRICTED:send_to_char("You can't use this command from within another command.\r\n", ch);
            return;
        case SUB_MPROG_EDIT:
            if (!ch->dest_buf)
            {
