...
void fread_mssp_info(FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (UPPER(word[0]))
        {
            ...
            case 'A':KEY("AdultMaterial", mssp_info->adultMaterial, fread_number(fp));
                KEY("Ansi", mssp_info->ansi, fread_number(fp));
                break;

            case 'C':KEY("Contact", mssp_info->contact, fread_string_nohash(fp));
            ...
            case 'N':KEY("NewbieFriendly", mssp_info->newbieFriendly, fread_number(fp));
                break;

            case 'P':KEY("Pay2Play", mssp_info->pay2play, fread_number(fp));
            ...
            case 'Q':KEY("QuestSystem", mssp_info->questSystem, fread_string_nohash(fp));
                break;

            case 'R':KEY("RolePlaying", mssp_info->roleplaying, fread_string_nohash(fp));
                break;

            case 'S':KEY("SSL", mssp_info->ssl, fread_number(fp));
                KEY("Status", mssp_info->status, fread_string_nohash(fp));
                KEY("SubGenre", mssp_info->subgenre, fread_string_nohash(fp));
                break;

            case 'T':KEY("TrainingSystem", mssp_info->trainingSystem, fread_string_nohash(fp));
                break;

            case 'V':KEY("Vt100", mssp_info->vt100, fread_number(fp));
                break;

            case 'W':KEY("Website", mssp_info->website, fread_string_nohash(fp));
                KEY("WorldOriginality", mssp_info->worldOriginality, fread_string_nohash(fp));
                KEY("Worlds", mssp_info->worlds, fread_number(fp));
                break;

            case 'X':KEY("Xterm256", mssp_info->xterm256, fread_number(fp));
                break;
        }
        if (!fMatch)
...
void do_setmssp(CHAR_DATA* ch, const char* argument)
{
    ...
    if (!argument || (argument[0] == '\0'))
    {
        ...
        send_to_char(
            "mudname        hostname          contact            icon             language          location\r\n",
            ch
        );
    ...
    else if (!str_cmp(arg1, "world_originality"))
    {
            ...
            str_cmp(argument, "All Original"))
        {
            ch_printf(
                ch,
                "Valid choices for %s are: All Stock, Mostly Stock, Mostly Original or All Original\r\n",
                arg1
            );
            return;
        }
...
void send_mssp_data(DESCRIPTOR_DATA* d)
{
    ...
    mssp_reply(d, "NAME", "%s", mssp_info->mudname);
    ...
    mssp_reply(d, "SKILLS", "%d", MAX_SKILL);
