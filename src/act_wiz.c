...
void echo_to_room(
    short AT_COLOR, ROOM_INDEX_DATA* room,
    const char* argument
)
{
    CHAR_DATA* vic;
...
void do_ostat(CHAR_DATA* ch, const char* argument)
{
    ...
    ch_printf(
        ch, "Vnum: %d.  Type: %s.  Count: %d  Gcount: %d\r\n",
        obj->pIndexData->vnum, item_type_name(obj), obj->pIndexData->count, obj->count
    );

    ch_printf(
        ch, "Serial#: %d  TopIdxSerial#: %d  TopSerial#: %d\r\n",
...
void do_mstat(CHAR_DATA* ch, const char* argument)
{
    ...
    if (get_trust(ch) >= LEVEL_GOD && !IS_NPC(victim) && victim->desc)
        ch_printf(
            ch, "Host: %s   Descriptor: %d   Trust: %d   AuthedBy: %s\r\n",
    ...
    if (!IS_NPC(victim) && victim->pcdata->release_date != 0)
        ch_printf(
            ch,
            "Helled until %24.24s by %s.\r\n",
            ctime(&victim->pcdata->release_date),
            victim->pcdata->helled_by
        );

    ch_printf(
        ch, "Vnum: %d   Sex: %s   Room: %d   Count: %d  Killed: %d\r\n",
    ...
    ch_printf(
        ch, "Str: %d  Int: %d  Wis: %d  Dex: %d  Con: %d  Cha: %d  Lck: %d  Frc: %d\r\n",
        get_curr_str(victim),
        get_curr_int(victim),
        get_curr_wis(victim),
        get_curr_dex(victim),
    ...
    ch_printf(
        ch, "Hitroll: %d   Damroll: %d   Position: %d   Wimpy: %d \r\n",
        GET_HITROLL(victim), GET_DAMROLL(victim), victim->position, victim->wimpy
    );
    ch_printf(
        ch, "Fighting: %s    Master: %s    Leader: %s\r\n",
    ...
    if (!IS_NPC(victim))
        ...
        ch_printf(
            ch, "Hit dice: %dd%d+%d.  Damage dice: %dd%d+%d.\r\n",
...
void do_owhere(CHAR_DATA* ch, const char* argument)
{
    ...
    if (arg1[0] != '\0' && !str_prefix(arg1, "nesthunt"))
    {
        ...
        if (obj->carried_by)
            snprintf(
                buf + strlen(buf), (MAX_STRING_LENGTH - strlen(buf)), "invent [%5d] %s\r\n",
    ...
    */
    ...
    if (!rename(pfile, backup))
    {
        ...
        else if (errno != ENOENT)
        {
            ch_printf(
                ch,
                "Unknown error #%d - %s (immortal data). Report to smaugmuds.afkmods.com\r\n",
                errno,
                strerror(errno));
        ...
        for (pArea = first_build; pArea; pArea = pArea->next)
        {
            if (!str_cmp(pArea->filename, areafile))
            {
                ...
                else if (errno != ENOENT)
                {
                    ch_printf(
                        ch,
                        "Unknown error #%d - %s (area data). Report to smaugmuds.afkmods.com\r\n",
                        errno,
                        strerror(errno));
                    snprintf(ebuf, MAX_STRING_LENGTH, "%s destroying %s", ch->name, buildfile);
                    perror(ebuf);
...
void do_hell(CHAR_DATA* ch, const char* argument)
{
    ...
    {
        ch_printf(
            ch, "They are already in hell until %24.24s, by %s.\r\n",
