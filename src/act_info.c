...
void show_list_to_char(OBJ_DATA* list, CHAR_DATA* ch, bool fShort, bool fShowNothing)
{
    ...
    */
    for (iShow = 0; iShow < nShow; iShow++)
    {
        ...
        {
            default:set_char_color(AT_OBJECT, ch);
                break;
            case ITEM_BLOOD:set_char_color(AT_BLOOD, ch);
                break;
            case ITEM_MONEY:
            case ITEM_TREASURE:set_char_color(AT_YELLOW, ch);
                break;
            case ITEM_FOOD:set_char_color(AT_HUNGRY, ch);
                break;
            case ITEM_DRINK_CON:
            case ITEM_FOUNTAIN:set_char_color(AT_THIRSTY, ch);
                break;
            case ITEM_FIRE:set_char_color(AT_FIRE, ch);
                break;
            case ITEM_SCROLL:
            case ITEM_WAND:
            case ITEM_STAFF:set_char_color(AT_MAGIC, ch);
                break;
        }
        if (fShowNothing)
            send_to_char("     ", ch);
...
void show_visible_affects_to_char(CHAR_DATA* victim, CHAR_DATA* ch)
{
    ...
    if (IS_AFFECTED(victim, AFF_SANCTUARY))
    {
        if (IS_GOOD(victim))
        {
            ...
            ch_printf(
                ch, "%s glows with an aura of divine radiance.\r\n",
        ...
        else if (IS_EVIL(victim))
        {
            set_char_color(AT_WHITE, ch);
            ch_printf(
                ch, "%s shimmers beneath an aura of dark energy.\r\n",
                IS_NPC(victim) ? capitalize(victim->short_descr) : (victim->name));
        }
        else
        {
            set_char_color(AT_WHITE, ch);
            ch_printf(
                ch, "%s is shrouded in flowing shadow and light.\r\n",
                IS_NPC(victim) ? capitalize(victim->short_descr) : (victim->name));
        }
    }
    if (IS_AFFECTED(victim, AFF_FIRESHIELD))
    {
        ...
        ch_printf(
            ch, "%s is engulfed within a blaze of mystical flame.\r\n",
            IS_NPC(victim) ? capitalize(victim->short_descr) : (victim->name));
    }
    if (IS_AFFECTED(victim, AFF_SHOCKSHIELD))
    {
        ...
        ch_printf(
            ch, "%s is surrounded by cascading torrents of energy.\r\n",
            IS_NPC(victim) ? capitalize(victim->short_descr) : (victim->name));
    }
    /*Scryn 8/13*/
    if (IS_AFFECTED(victim, AFF_ICESHIELD))
    {
        ...
        ch_printf(
            ch, "%s is ensphered by shards of glistening ice.\r\n",
            IS_NPC(victim) ? capitalize(victim->short_descr) : (victim->name));
    }
    if (IS_AFFECTED(victim, AFF_CHARM))
    {
        ...
        ch_printf(
            ch, "%s looks ahead free of expression.\r\n",
            IS_NPC(victim) ? capitalize(victim->short_descr) : (victim->name));
    }
...
void show_char_to_char_0(CHAR_DATA* victim, CHAR_DATA* ch)
{
    ...
    */
    ...
    {
        case POS_DEAD:mudstrlcat(buf, " is DEAD!!", MAX_STRING_LENGTH);
            break;
        case POS_MORTAL:mudstrlcat(buf, " is mortally wounded.", MAX_STRING_LENGTH);
            break;
        case POS_INCAP:mudstrlcat(buf, " is incapacitated.", MAX_STRING_LENGTH);
            break;
        case POS_STUNNED:mudstrlcat(buf, " is lying here stunned.", MAX_STRING_LENGTH);
            break;
        case POS_SLEEPING:
        ...
        case POS_STANDING:
            ...
            break;
        case POS_SHOVE:mudstrlcat(buf, " is being shoved around.", MAX_STRING_LENGTH);
            break;
        case POS_DRAG:mudstrlcat(buf, " is being dragged around.", MAX_STRING_LENGTH);
            break;
        case POS_MOUNTED:mudstrlcat(buf, " is here, upon ", MAX_STRING_LENGTH);
            if (!victim->mount)
            ...
            break;
        case POS_FIGHTING:mudstrlcat(buf, " is here, fighting ", MAX_STRING_LENGTH);
            if (!victim->fighting)
                mudstrlcat(buf, "thin air???", MAX_STRING_LENGTH);
    ...
    */
    if ((obj = get_obj_here(ch, arg)) != nullptr)
    {
        ...
        switch (obj->item_type)
        {
            ...
            case ITEM_WEAPON:dam = INIT_WEAPON_CONDITION - obj->value[0];
                mudstrlcpy(buf, "As you look more closely, you notice that it is ", MAX_STRING_LENGTH);
                if (dam == 0)
                    mudstrlcat(buf, "in superb condition.", MAX_STRING_LENGTH);
                else if (dam == 1)
            ...
            case ITEM_CORPSE_NPC:
            {
                ...
                switch (timerfrac)
                {
                    default:send_to_char("This corpse has recently been slain.\r\n", ch);
                        break;
                    case 4:send_to_char("This corpse was slain a little while ago.\r\n", ch);
                        break;
                    case 3:send_to_char("A foul smell rises from the corpse, and it is covered in flies.\r\n", ch);
                        break;
                    case 2:
                        send_to_char(
                        ...
                        break;
                    case 1:
                    case 0:send_to_char("Little more than bones, there isn't much left of this corpse.\r\n", ch);
                        break;
                }
            }
                if (IS_OBJ_STAT(obj, ITEM_COVERING))
            ...
            case ITEM_DROID_CORPSE:
            {
                ...
                switch (timerfrac)
                {
                    default:send_to_char("These remains are still smoking.\r\n", ch);
                        break;
                    case 4:send_to_char("The parts of this droid have cooled down completely.\r\n", ch);
                        break;
                    case 3:send_to_char("The broken droid components are beginning to rust.\r\n", ch);
                        break;
                    case 2:send_to_char("The pieces are completely covered in rust.\r\n", ch);
                        break;
                    case 1:
                    case 0:send_to_char("All that remains of it is a pile of crumbling rust.\r\n", ch);
                        break;
                }
            }
            ...
            case ITEM_DRINK_CON:send_to_char("When you look inside, you see:\r\n", ch);
                EXA_prog_trigger = FALSE;
    ...
    for (pexit = ch->in_room->first_exit; pexit; pexit = pexit->next)
    {
        if (pexit->to_room && !IS_SET(pexit->exit_info, EX_HIDDEN))
        {
            ...
            if (!fAuto)
            {
                ...
                {
                    snprintf(
                        buf + strlen(buf), (MAX_STRING_LENGTH - strlen(buf)), "%-5s - %s\r\n",
...
void do_who(CHAR_DATA* ch, const char* argument)
{
    ...
    */
    ...
    for (d = last_descriptor; d; d = d->prev)
    {
        ...
        {
            default:break;
            case 200:race = "The Ghost in the Machine";
                break;
            case MAX_LEVEL - 0:race = "President";
                break;
            case MAX_LEVEL - 1:race = "Executive";
                break;
            case MAX_LEVEL - 2:race = "Senator";
                break;
            case MAX_LEVEL - 3:race = "Engineer";
                break;
            case MAX_LEVEL - 4:race = "Retired";
                break;
        }
        ...
        snprintf(
            buf, MAX_STRING_LENGTH + 1, "%s %s%s%s%s %s%s%s\r\n",
            race,
            invis_str,
       ...
       */
        switch (cur_who->type)
        {
            case WT_MORTAL:cur_who->next = first_mortal;
                first_mortal = cur_who;
                break;
            case WT_IMM:cur_who->next = first_imm;
                first_imm = cur_who;
                break;
            case WT_NEWBIE:cur_who->next = first_newbie;
...
void do_compare(CHAR_DATA* ch, const char* argument)
{
    ...
    else
    {
        ...
        {
            default:msg = "You can't compare $p and $P.";
                break;

            case ITEM_ARMOR:value1 = obj1->value[0];
                value2             = obj2->value[0];
                break;

            case ITEM_WEAPON:value1 = obj1->value[1] + obj1->value[2];
...
void do_whois(CHAR_DATA* ch, const char* argument)
{
    ...
    if (IS_IMMORTAL(ch))
    {
        ...
        ch_printf(
            ch, "%s has killed %d mobiles, and been killed by a mobile %d times.\r\n",
            victim->name, victim->pcdata->mkills, victim->pcdata->mdeaths
        );
        if (victim->pcdata->pkills || victim->pcdata->pdeaths)
            ch_printf(
                ch, "%s has killed %d players, and been killed by a player %d times.\r\n",
        ...
        ch_printf(
            ch, "%s is %shelled at the moment.\r\n",
            victim->name, (victim->pcdata->release_date == 0) ? "not " : ""
        );

        if (victim->pcdata->release_date != 0)
            ch_printf(
                ch, "%s was helled by %s, and will be released on %24.24s.\r\n",
