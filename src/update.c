...
int hit_gain(CHAR_DATA* ch)
{
    ...
    else
    {
        ...
        switch (ch->position)
        {
            ...
            case POS_SLEEPING: gain += (int)(get_curr_con(ch) * 1.5);
                break;
            case POS_RESTING:gain += get_curr_con(ch);
                break;
        }
...
int mana_gain(CHAR_DATA* ch)
{
    ...
    else
    {
        ...
        switch (ch->position)
        {
            case POS_SLEEPING:gain += get_curr_int(ch) * 3;
                break;
            case POS_RESTING: gain += (int)(get_curr_int(ch) * 1.5);
                break;
        }
...
int move_gain(CHAR_DATA* ch)
{
    ...
    else
    {
        ...
        switch (ch->position)
        {
            ...
            case POS_SLEEPING:gain += get_curr_dex(ch) * 2;
                break;
            case POS_RESTING:gain += get_curr_dex(ch);
                break;
        }
    ...
    if (ch->pcdata->condition[iCond] == 0)
    {
        switch (iCond)
        {
            ...
            case COND_DRUNK:
                ...
                retcode = rNONE;
                break;
            default:bug("%s: invalid condition type %d", __func__, iCond);
                retcode = rNONE;
                break;
        }
    }
    ...
    alg::for_each(characters | view::reverse, [&](auto* ch)
    {
       ...
       */
        if (!IS_SET(ch->act, ACT_RUNNING)
            ...
            &&
            (pexit = get_exit(ch->in_room, door)) != nullptr
            &&
            pexit->to_room
            &&
            ...
            && !IS_SET(pexit->to_room->room_flags, ROOM_NO_MOB))
        {
            ...
            for (auto* rch : ch->in_room->persons)
            {
                if (is_fearing(ch, rch))
                {
                    ...
                    {
                        case 0:snprintf(buf, MAX_STRING_LENGTH, "Get away from me, %s!", rch->name);
                            break;
                        case 1:snprintf(buf, MAX_STRING_LENGTH, "Leave me be, %s!", rch->name);
                            break;
                        case 2:snprintf(buf, MAX_STRING_LENGTH, "%s is trying to kill me!  Help!", rch->name);
                            break;
                        case 3:snprintf(buf, MAX_STRING_LENGTH, "Someone save me from %s!", rch->name);
                            break;
                    }
                    do_yell(ch, buf);
                    found = TRUE;
...
void weather_update(void)
{
    ...
    */
    ...
    {
        default:bug("%s: bad sky %d.", __func__, weather_info.sky);
       ...
       */
        ...
        if (!IS_NPC(ch) && ch->top_level < LEVEL_IMMORTAL)
        {
            ...
            if (ch->pcdata->condition[COND_FULL] > 1)
            {
                switch (ch->position)
                {
                    case POS_SLEEPING:better_mental_state(ch, 4);
                        break;
                    case POS_RESTING:better_mental_state(ch, 3);
                        break;
                    case POS_SITTING:
                    case POS_MOUNTED:better_mental_state(ch, 2);
                        break;
                    case POS_STANDING:better_mental_state(ch, 1);
                        break;
                    case POS_FIGHTING:
                        if (number_bits(2) == 0)
            ...
            if (ch->pcdata->condition[COND_THIRST] > 1)
            {
                switch (ch->position)
                {
                    case POS_SLEEPING:better_mental_state(ch, 5);
                        break;
                    case POS_RESTING:better_mental_state(ch, 3);
                        break;
                    case POS_SITTING:
            ...
            if (ch->in_room)
                ...
                {
                    default:gain_condition(ch, COND_THIRST, -1);
                        break;
                    case SECT_DESERT:gain_condition(ch, COND_THIRST, -2);
                        break;
            ...
            if (ch->mental_state >= 30)
                switch ((ch->mental_state + 5) / 10)
                {
                    ...
                    case 9:send_to_char("You are ONE with the universe.\r\n", ch);
                        ...
                        break;
                    case 10:send_to_char("You feel the end is near.\r\n", ch);
...
void obj_update(void)
{
    ...
    for (obj = last_object; obj; obj = gobj_prev)
    {
        ...
        {
            bug("%s: first_object->prev != nullptr... fixed", __func__);
            obj->prev = nullptr;
        }
        gobj_prev = obj->prev;
       ...
       */
        ...
        {
            default:message = "$p has depleted itself.";
                AT_TEMP     = AT_PLAIN;
                break;

            case ITEM_GRENADE:explode(obj);
                return;
                break;

            case ITEM_PORTAL:message = "$p winks out of existence.";
                remove_portal(obj);
            ...
            case ITEM_FOUNTAIN:message = "$p dries up.";
                AT_TEMP                = AT_BLUE;
                break;
            case ITEM_CORPSE_NPC:message = "$p decays into dust and blows away.";
                AT_TEMP                  = AT_OBJECT;
                break;
            case ITEM_DROID_CORPSE:message = "$p rusts away into oblivion.";
                AT_TEMP                    = AT_OBJECT;
                break;
            case ITEM_CORPSE_PC:message = "$p decays into dust and is blown away...";
                AT_TEMP                 = AT_MAGIC;
                break;
            case ITEM_FOOD:message = "$p is devoured by a swarm of maggots.";
                AT_TEMP            = AT_HUNGRY;
                break;
            case ITEM_BLOOD:message = "$p slowly seeps into the ground.";
                AT_TEMP             = AT_BLOOD;
                break;
            case ITEM_BLOODSTAIN:message = "$p dries up into flakes and blows away.";
                AT_TEMP                  = AT_BLOOD;
                break;
            case ITEM_SCRAPS:message = "$p crumbles and decays into nothing.";
                AT_TEMP              = AT_OBJECT;
                break;
            case ITEM_FIRE:
                if (obj->in_room)
    ...
    alg::for_each(descriptors, [&](auto* d)
    {
        ...
        alg::for_each(wch->in_room->persons, [&](auto* ch)
        {
            ...
            if (IS_NPC(ch) && IS_SET(ch->attacks, ATCK_BACKSTAB))
            {
                ...
                if (!ch->mount
                    && (obj = get_eq_char(ch, WEAR_WIELD)) != nullptr
...
void reboot_check(time_t reset)
{
    ...
    if (new_boot_time_t <= current_time)
    {
        ...
        echo_to_all(
            AT_YELLOW, "You are forced from these realms by a strong "
    ...
    switch (++auction->going)   /* increase the going state */
    {
        ...
        case 2: /* going twice */
            if (auction->bet > auction->starting)
                snprintf(
                    buf, MAX_STRING_LENGTH, "%s: going %s for %d.", auction->item->short_descr,
                    ((auction->going == 1) ? "once" : "twice"), auction->bet
                );
            else
                snprintf(
                    buf, MAX_STRING_LENGTH, "%s: going %s (bid not received yet).", auction->item->short_descr,
        ...
        case 3: /* SOLD! */
            ...
            {
                snprintf(
                    buf, MAX_STRING_LENGTH, "%s sold to %s for %d.",
                ...
                act(
                    AT_ACTION, "The auctioneer materializes before you, and hands you $p.",
                    auction->buyer, auction->item, nullptr, TO_CHAR
                );
                act(
                    AT_ACTION, "The auctioneer materializes before $n, and hands $m $p.",
                    auction->buyer, auction->item, nullptr, TO_ROOM
                );
            ...
            {
                snprintf(
                    buf, MAX_STRING_LENGTH, "No bids received for %s - object has been removed from auction\r\n.",
                    auction->item->short_descr
                );
                talk_auction(buf);
                act(
                    AT_ACTION, "The auctioneer appears before you to return $p to you.",
                    auction->seller, auction->item, nullptr, TO_CHAR
                );
                act(
                    AT_ACTION, "The auctioneer appears before $n to return $p to $m.",
                    auction->seller, auction->item, nullptr, TO_ROOM
                );
                if ((auction->seller->carry_weight + get_obj_weight(auction->item)) > can_carry_w(auction->seller))
                {
                    ...
                    act(
                        AT_PLAIN, "$n drops $p as it is too much extra weight"
                                  " for $m with everything else.", auction->seller, auction->item, nullptr, TO_ROOM
                    );
