...
void imc_new_channel(
    const char* chan, const char* owner, const char* ops, const char* invite, const char* exclude,
...
void imc_write_buffer(const char* txt)
{
    ...
    */
    while (this_imcmud->outtop + length >= this_imcmud->outsize)
    {
        ...
        IMCRECREATE(this_imcmud->outbuf,
        char, this_imcmud->outsize );
    }
    ...
    */
    strncpy(
        this_imcmud->outbuf + this_imcmud->outtop,
        output,
        length
    );   /* Leave this one alone! BAD THINGS(TM) will happen if you don't! */
...
void update_imchistory(IMC_CHANNEL* channel, char* message)
{
    ...
    for (x = 0; x < MAX_IMCHISTORY; x++)
    {
        if (channel->history[x] == nullptr)
        {
            ...
            snprintf(
                buf, LGST, "~R[%-2.2d/%-2.2d %-2.2d:%-2.2d] ~G%s",
...
const char* get_local_chanwho(IMC_CHANNEL* c)
{
    ...
    snprintf(
        buf, IMC_BUFF_SIZE, "The following people are listening to %s on %s:\r\n\r\n",
...
char* imccenterline(const char* string, int length)
{
    ...
    */
    snprintf(
        outbuf, 400, "%*s%s%*s", (amount / 2), "", string,
...
char* imc_assemble_who(void)
{
    ...
    char           plrheader[SMST], immheader[SMST], rank[SMST], flags[SMST], name[SMST], title[SMST], plrline[SMST],
                   immline[SMST];
...
void imc_process_finger(const char* from, const char* type)
{
    ...
    if (!IMCIS_SET(IMCFLAG(victim), IMC_PRIVACY))
        snprintf(
            buf + strlen(buf), IMC_BUFF_SIZE - strlen(buf),
            "~cEmail   : ~W%s\r\n"
            "~cHomepage: ~W%s\r\n"
            "~cICQ     : ~W%d\r\n"
            "~cAIM     : ~W%s\r\n"
    ...
    snprintf(
        buf + strlen(buf), IMC_BUFF_SIZE - strlen(buf), "~W%s\r\n",
...
void imc_parse_packet(const char* packet)
{
    ...
    */
    ...
    if (!pfun)
    {
        ...
        {
            imclog(
                "PACKET: From %s, Seq %lu, Route %s, Type %s, To %s, EXTRA %s",
...
bool imc_read_socket(void)
{
    ...
    for (;;)
    {
        ...
        if (++loop_count >
            100) // Yay hackish error traps! 100 loops likely means she's stuck. Break out with whatever is there.
            break;
...
void imc_char_login(CHAR_DATA* ch)
{
    ...
    if (this_imcmud->state != IMC_ONLINE)
    {
        if (IMCPERM(ch) >= IMCPERM_IMM && imcwait == -2)
            imc_to_char(
                "~RThe IMC2 connection is down. Attempts to reconnect were abandoned due to excessive failures.\r\n",
                ch
            );
        return;
    }
...
bool imc_loadchar(CHAR_DATA* ch, FILE* fp, const char* word)
{
    ...
    switch (word[0])
    {
        case 'I':IMCKEY("IMCPerm", IMCPERM(ch), imcfread_number(fp));
...
void imc_load_pfile( CHAR_DATA * ch, char *tag, int num, char *line )
{
   ...
   if( !strcmp( tag, "IMCLSN" ) )
   {
      ...
      if( IMC_LISTEN( ch ) != nullptr && imc_active == IA_UP )
      {
         IMC_CHANNEL *channel = nullptr;
   ...
   if( !strcmp( tag, "IMCDNY" ) )
   {
      ...
      if( IMC_DENY( ch ) != nullptr && imc_active == IA_UP )
      {
         IMC_CHANNEL *channel = nullptr;
...
void imc_readchannel(IMC_CHANNEL* channel, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            ...
            case 'C':IMCKEY("ChanName", channel->name, imcfread_line(fp));
...
void imc_readcolor(IMC_COLOR* color, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            ...
            case 'I':IMCKEY("IMCtag", color->imctag, imcfread_line(fp));
                break;

            case 'M':IMCKEY("Mudtag", color->mudtag, imcfread_line(fp));
                break;

            case 'N':IMCKEY("Name", color->name, imcfread_line(fp));
                break;
        }
        if (!fMatch)
            imcbug("%s: no match: %s", __func__, word);
...
void imc_readhelp(IMC_HELP_DATA* help, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            ...
            case 'N':IMCKEY("Name", help->name, imcfread_line(fp));
                break;
...
void imc_readcommand(IMC_CMD_DATA* cmd, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            ...
            case 'C':IMCKEY("Connected", cmd->connected, imcfread_number(fp));
                if (!strcasecmp(word, "Code"))
                {
                    word = imcfread_word(fp);
                    cmd->function = imc_function(word);
                    if (cmd->function == nullptr)
                        imcbug("%s: Command %s loaded with invalid function. Set to nullptr.", __func__, cmd->name);
                    fMatch = TRUE;
                    break;
                }
                break;

            case 'N':IMCKEY("Name", cmd->name, imcfread_line(fp));
                break;
...
void imc_readucache(IMCUCACHE_DATA* user, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            ...
            case 'N':IMCKEY("Name", user->name, imcfread_line(fp));
                break;

            case 'S':IMCKEY("Sex", user->gender, imcfread_number(fp));
                break;

            case 'T':IMCKEY("Time", user->time, imcfread_number(fp));
                break;
...
void imcfread_config_file(FILE* fin)
{
    ...
    for (;;)
    {
        ...
        switch (word[0])
        {
            ...
            case 'A':IMCKEY("Autoconnect", this_imcmud->autoconnect, imcfread_number(fin));
                IMCKEY("AdminLevel", this_imcmud->adminlevel, imcfread_number(fin));
                break;

            case 'C':IMCKEY("ClientPwd", this_imcmud->clientpw, imcfread_line(fin));
                break;
            ...
            case 'I':IMCKEY("Implevel", this_imcmud->implevel, imcfread_number(fin));
            ...
            case 'L':IMCKEY("LocalName", this_imcmud->localname, imcfread_line(fin));
                break;

            case 'M':IMCKEY("MinImmLevel", this_imcmud->immlevel, imcfread_number(fin));
                IMCKEY("MinPlayerLevel", this_imcmud->minlevel, imcfread_number(fin));
                break;

            case 'R':IMCKEY("RouterAddr", this_imcmud->rhost, imcfread_line(fin));
                IMCKEY("RouterPort", this_imcmud->rport, imcfread_number(fin));
                break;

            case 'S':IMCKEY("ServerPwd", this_imcmud->serverpw, imcfread_line(fin));
...
bool imc_server_connect(void)
{
    ...
    */
    if (this_imcmud->sha256)
    {
       ...
       */
        ...
        {
            snprintf(
                buf, LGST, "PW %s %s version=%d autosetup %s SHA256",
    ...
    */
        ...
        snprintf(
            buf, LGST, "PW %s %s version=%d autosetup %s",
            this_imcmud->localname, this_imcmud->clientpw, IMC_VERSION, this_imcmud->serverpw
        );
...
IMC_CMD( imccommand )
           {
               ...
               if ( !cmd[0] || !chan[0] )
               {
                   ...
                   imc_to_char(
                       "Command access will depend on your privledges and what each individual server allows.\r\n",
                       ch
                   );
                   return;
               }
...
IMC_CMD( imcsetup )
    ...
    */
               ...
               if ( !strcasecmp( imccmd, "rename" ))
               {
                   ...
                   {
                       imc_to_char(
                           "Missing 'newname' argument for 'imcsetup rename'\r\n",
                           ch
                       ); /* Lets be more kind! -- X */
                       imc_to_char(
                           "Syntax: imcsetup rename <local channel> <newname>\r\n",
                           ch
       ...
       */
               ...
               if ( !strcasecmp( imccmd, "regformat" ))
               {
                   ...
                   {
                       imc_to_char(
                           "Syntax: imcsetup regformat <localchannel|all> <string>\r\n",
                           ch
                       ); /* Syntax Fix -- Xorith */
                       return;
                   }
               ...
               if ( !strcasecmp( imccmd, "emoteformat" ))
               {
                   ...
                   {
                       imc_to_char(
                           "Syntax: imcsetup emoteformat <localchannel|all> <string>\r\n",
                           ch
                       );  /* Syntax Fix -- Xorith */
                       return;
                   }
...
IMC_CMD( imcchanlist )
       ...
       */
               ...
               imc_printf(
                   ch, "~c%-15.15s ~%c%-*.*s ~B%-15.15s ~b%-7s %s\r\n", c->name, col,
...
IMC_CMD( imcfinger )
           {
               ...
               if ( !strcasecmp( arg, "privacy" ))
               {
                   ...
                   else
                   {
                       ...
                       imc_to_char(
                           "Privacy flag enabled. Your information will no longer be visible on imcfinger.\r\n",
                           ch
                       );
                   }
                   return;
...
IMC_CMD( imcconfig )
           {
               ...
               if ( arg1[0] == '\0' )
               {
                   ...
                   imc_to_char(
                       "~wMinImmLevel    : ~GSets the level at which immortal commands become available.\r\n",
                       ch
                   );
                   imc_to_char(
                       "~wAdminlevel     : ~GSets the level at which administrative commands become available.\r\n",
                       ch
                   );
                   imc_to_char(
                       "~wImplevel       : ~GSets the level at which immplementor commands become available.\r\n",
                       ch
                   );
...
IMC_CMD( imcpermset )
    ...
    */
           if ( IMCPERM( victim ) == permvalue )
           {
               imc_printf(
                   ch,
                   "%s already has a permission level of %s.\r\n",
                   CH_IMCNAME(victim),
           ...
           if ( IMC_LISTEN( victim ) != nullptr && this_imcmud->state == IMC_ONLINE )
           {
               ...
               while (1)
               {
                   ...
                   if (channel && IMCPERM(victim) < channel->level)
                   {
                       imc_removename(&IMC_LISTEN(victim), arg);
                       imc_printf(
                           ch, "~WRemoving '%s' level channel: '%s', exceeding new permission of '%s'\r\n",
...
IMC_CMD( imccedit )
           ...
           {
               imc_to_char(
                   "Usage: imccedit <command> <create|delete|alias|rename|code|permission|connected> <field>.\r\n",
                   ch
               );
               return;
           }
    ...
    */
           ...
           if ( !strcasecmp( option, "perm" ) || !strcasecmp( option, "permission" ))
           {
               ...
               cmd->level = permvalue;
               imc_printf(
                   ch, "~gCommand ~W%s ~gpermission level has been changed to ~W%s.\r\n", cmd->name,
                   imcperm_names[permvalue]
               );
               imc_savecommands();
               return;
    ...
    while (*format != '\0')
    {
        ...
        else
        {
            switch (*format)
            {
                default:i = " !!!!! ";
                    break;
                case 'n':i = imc_makename(CH_IMCNAME(ch), this_imcmud->localname);
                    break;
                case 'N':i = CH_IMCNAME(vic);
                    break;
                ...
                case 'k':imcone_argument(CH_IMCNAME(ch), tmp_str);
                    i = (char*)tmp_str;
                    break;
                case 'K':imcone_argument(CH_IMCNAME(vic), tmp_str);
                    i = (char*)tmp_str;
                    break;
            }
        }
    ...
    */
    ...
    if (IMCPERM(ch) >= IMCPERM_ADMIN && !strcasecmp(argument, "log"))
    {
        if (!IMCIS_SET(c->flags, IMCCHAN_LOG))
        {
            ...
            imc_printf(
                ch, "~RFile logging enabled for %s, PLEASE don't forget to undo this when it isn't needed!\r\n",
                c->local_name
            );
        }
        else
    ...
    {
        imc_printf(
            ch, "You are not currently listening to %s. Use the imclisten command to listen to this channel.\r\n",
            c->local_name
        );
        return TRUE;
    }
    ...
    switch (argument[0])
    {
        ...
        case '@':
          ...
          */
            ...
            break;
        default:imc_sendmessage(c, CH_IMCNAME(ch), color_mtoi(argument), 0);
            break;
    }
    return TRUE;
}
