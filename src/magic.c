...
int rd_parse(CHAR_DATA* ch, int level, char* texp)
{
    ...
    */
    ...
    if (len == 1 && isalpha(texp[0]))
        switch (texp[0])
        {
            ...
            case 'l':return level;
            case 'H':
            case 'h':return ch->hit;
            case 'M':
            case 'm':return ch->mana;
            case 'V':
            case 'v':return ch->move;
            case 'S':
            case 's':return get_curr_str(ch);
            case 'I':
            case 'i':return get_curr_int(ch);
            case 'W':
            case 'w':return get_curr_wis(ch);
            case 'X':
            case 'x':return get_curr_dex(ch);
            case 'C':
            case 'c':return get_curr_con(ch);
            case 'A':
            case 'a':return get_curr_cha(ch);
            case 'U':
            case 'u':return get_curr_lck(ch);
            case 'Y':
            case 'y':return get_age(ch);
        }
    ...
    */
    ...
    switch (operation)
    {
        case '-':total -= rd_parse(ch, level, sexp[1]);
            break;
        case '+':total += rd_parse(ch, level, sexp[1]);
            break;
        case '*':total *= rd_parse(ch, level, sexp[1]);
        ...
        case '%':total %= rd_parse(ch, level, sexp[1]);
            break;
        case 'd':
        case 'D':total = dice(total, rd_parse(ch, level, sexp[1]));
            break;
        case '^':
        {
       ...
       */
        ...
        if (obj)
        {
            if (val >= 0 && val < 6)
            {
                ...
                {
                    act(AT_MAGIC, "$p glows briefly, then disappears in a puff of smoke!", ch, obj, nullptr, TO_CHAR);
                    act(AT_MAGIC, "$p glows briefly, then disappears in a puff of smoke!", ch, obj, nullptr, TO_ROOM);
                    extract_obj(obj);
                }
                else
                    act(AT_MAGIC, "$p glows briefly and a whisp of smoke rises from it.", ch, obj, nullptr, TO_CHAR);
            }
            else if (consume)
            {
                separate_obj(obj);
                act(AT_MAGIC, "$p glows brightly, then disappears in a puff of smoke!", ch, obj, nullptr, TO_CHAR);
                act(AT_MAGIC, "$p glows brightly, then disappears in a puff of smoke!", ch, obj, nullptr, TO_ROOM);
                extract_obj(obj);
            }
            else
            {
                ...
                obj->count = 1;
                act(AT_MAGIC, "$p glows briefly.", ch, obj, nullptr, TO_CHAR);
                obj->count = count;
            }
        }
    }
...
void* locate_targets(CHAR_DATA* ch, char* arg, int sn, CHAR_DATA** victim, OBJ_DATA** obj)
{
    ...
    *obj    = nullptr;
    ...
    switch (skill->target)
    {
        default:bug("%s: bad target for sn %d.", __func__, sn);
            return &pAbort;
    ...
    switch (ch->substate)
    {
        default:
          ...
          */
            if (ch->position < skill->minimum_position)
            {
                switch (ch->position)
                {
                    default:send_to_char("You can't concentrate enough.\r\n", ch);
                        break;
                    case POS_SITTING:send_to_char("You can't summon enough energy sitting down.\r\n", ch);
                        break;
                    case POS_RESTING:send_to_char("You're too relaxed to cast that spell.\r\n", ch);
                        break;
                    case POS_FIGHTING:send_to_char("You can't concentrate enough while fighting!\r\n", ch);
                        break;
                    case POS_SLEEPING:send_to_char("You dream about great feats of magic.\r\n", ch);
                        break;
                }
                return;
            }
        ...
        case 1:sn      = ch->tempnum;
            ...
            if (!ch->dest_buf || !IS_VALID_SN(sn) || skill->type != SKILL_SPELL)
            {
                ...
                bug("%s: ch->dest_buf nullptr or bad sn (%d)", __func__, sn);
                return;
            }
            mana = IS_NPC(ch) ? 0 : skill->min_mana;
                ...
                if (cnt >= skill->participants)
                {
                    for (tmp = ch->in_room->first_person; tmp && tmp != ch; tmp = tmp->next_in_room)
                    {
                            ...
                            && tmp->tempnum == sn && tmp->dest_buf && !str_cmp((const char*)tmp->dest_buf, staticbuf))
                        {
                            ...
                            act(AT_MAGIC, "$N channels $S energy into you!", ch, nullptr, tmp, TO_CHAR);
                            act(AT_MAGIC, "$N channels $S energy into $n!", ch, nullptr, tmp, TO_NOTVICT);
        ...
        switch (number_bits(2))
        {
            ...
            case 2: /* not enough time */
                if (ch->fighting)
                    ...
                    send_to_char("You lost your concentration.\r\n", ch);
                break;
            case 3:send_to_char("A disturbance in the force muddles your concentration.\r\n", ch);
                break;
        }
...
ch_ret obj_cast_spell(int sn, int level, CHAR_DATA* ch, CHAR_DATA* victim, OBJ_DATA* obj)
{
    ...
    */
        ...
        && skill->type != SKILL_HERB && !chance(ch, 95 + levdiff))
    {
        switch (number_bits(2))
        {
            case 0:failed_casting(skill, ch, victim, nullptr);
                break;
            case 1:act(AT_MAGIC, "The $t backfires!", ch, skill->name, victim, TO_CHAR);
                if (victim)
            ...
            case 2:failed_casting(skill, ch, victim, nullptr);
                break;
            case 3:act(AT_MAGIC, "The $t backfires!", ch, skill->name, victim, TO_CHAR);
                if (victim)
                    act(AT_MAGIC, "$n's $t backfires!", ch, skill->name, victim, TO_VICT);
                act(AT_MAGIC, "$n's $t backfires!", ch, skill->name, victim, TO_NOTVICT);
                return damage(ch, ch, number_range(1, level), TYPE_UNDEFINED);
...
ch_ret spell_create_food(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(AT_MAGIC, "$p suddenly appears.", ch, mushroom, nullptr, TO_ROOM);
    act(AT_MAGIC, "$p suddenly appears.", ch, mushroom, nullptr, TO_CHAR);
...
ch_ret spell_cure_poison(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (is_affected(victim, gsn_poison))
    {
        ...
        act(AT_MAGIC, "$N looks better.", ch, nullptr, victim, TO_NOTVICT);
        set_char_color(AT_MAGIC, victim);
...
ch_ret spell_dispel_evil(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (IS_GOOD(victim))
    {
        act(AT_MAGIC, "The light side protects $N.", ch, nullptr, victim, TO_ROOM);
        return rSPELL_FAILED;
    }

    if (IS_NEUTRAL(victim))
    {
        act(AT_MAGIC, "$N does not seem to be affected.", ch, nullptr, victim, TO_CHAR);
        return rSPELL_FAILED;
    }
...
ch_ret spell_dispel_magic(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    else if (victim->affected_by == race_table[victim->race].affected
...
ch_ret spell_enchant_weapon(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    */
    ...
    if (IS_GOOD(ch))
    {
        ...
        act(AT_BLUE, "$p glows blue.", ch, obj, nullptr, TO_CHAR);
    }
    else if (IS_EVIL(ch))
    {
        SET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
        act(AT_RED, "$p glows red.", ch, obj, nullptr, TO_CHAR);
    }
    else
    {
        SET_BIT(obj->extra_flags, ITEM_ANTI_EVIL);
        SET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
        act(AT_YELLOW, "$p glows yellow.", ch, obj, nullptr, TO_CHAR);
    }
...
ch_ret spell_energy_drain(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    {
        failed_casting(skill, ch, victim, nullptr);   /* SB */
        return rSPELL_FAILED;
    }
...
ch_ret spell_identify(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if ((obj = get_obj_carry(ch, target_name)) != nullptr)
    {
        ...
        switch (obj->item_type)
        {
            case ITEM_PILL:
            case ITEM_SCROLL:
            case ITEM_POTION:ch_printf(ch, "Level %d spells of:", obj->value[0]);
            ...
            case ITEM_AMMO:ch_printf(ch, "It has %d charges.\r\n", obj->value[0]);
                break;

            case ITEM_BOLT:ch_printf(ch, "It has %d energy bolts.\r\n", obj->value[0]);
                break;

            case ITEM_BATTERY:ch_printf(ch, "It has %d units of charge.\r\n", obj->value[0]);
                break;
            ...
            case ITEM_ARMOR:
                ...
                ch_printf(
                    ch, "Applied armor class is %d. ( based condition and location worn )\r\n",
    ...
    else if ((victim = get_char_room(ch, target_name)) != nullptr)
    {
        ...
        {
            ch_printf(
                ch, "%s appears to be between level %d and %d.\r\n",
                victim->name,
...
ch_ret spell_invis(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (victim)
    {
        ...
        act(AT_MAGIC, "$n fades out of existence.", victim, nullptr, nullptr, TO_ROOM);
        af.type      = sn;
    ...
    else
    {
        ...
        if (obj)
        {
            ...
            act(AT_MAGIC, "$p fades out of existence.", ch, obj, nullptr, TO_CHAR);
            return rNONE;
        }
    }
    ch_printf(ch, "You can't find %s!\r\n", target_name);
...
ch_ret spell_know_alignment(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(AT_MAGIC, msg, ch, nullptr, victim, TO_CHAR);
    return rNONE;
}
...
ch_ret spell_locate_object(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    for (obj = first_object; obj; obj = obj->next)
    {
        ...
        {
            bug(
                "%s: object [%d] %s is nested more than %d times!",
                __func__,
                obj->pIndexData->vnum,
                obj->short_descr,
                MAX_NEST
...
ch_ret spell_sleep(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    */
    ...
    if (IS_AWAKE(victim))
    {
        act(AT_MAGIC, "You feel very sleepy ..... zzzzzz.", victim, nullptr, nullptr, TO_CHAR);
        act(AT_MAGIC, "$n goes to sleep.", victim, nullptr, nullptr, TO_ROOM);
            ...
            switch (obj_lose->item_type)
            {
                case ITEM_ARMOR:
                    if (obj_lose->value[0] > 0)
                    {
                        ...
                        act(AT_DAMAGE, "$p is pitted and etched!", victim, obj_lose, nullptr, TO_CHAR);
                        if ((iWear = obj_lose->wear_loc) != WEAR_NONE)
                            victim->armor += apply_ac(
                                obj_lose,
                                iWear
                            );   // <-- victim is LOSING the benefit of obj->value[0]
                ...
                case ITEM_CONTAINER:separate_obj(obj_lose);
                    act(AT_DAMAGE, "$p fumes and dissolves!", victim, obj_lose, nullptr, TO_CHAR);
                    act(AT_OBJECT, "The contents of $p spill out onto the ground.", victim, obj_lose, nullptr, TO_ROOM);
                    act(AT_OBJECT, "The contents of $p spill out onto the ground.", victim, obj_lose, nullptr, TO_CHAR);
                    empty_obj(obj_lose, nullptr, victim->in_room);
        ...
        for (obj_lose = victim->first_carrying; obj_lose; obj_lose = obj_next)
        {
            ...
            switch (obj_lose->item_type)
            {
                ...
                case ITEM_CONTAINER:msg = "$p ignites and burns!";
                    break;
                case ITEM_POTION:msg = "$p bubbles and boils!";
                    break;
                case ITEM_SCROLL:msg = "$p crackles and burns!";
                    break;
                case ITEM_STAFF:msg = "$p smokes and chars!";
                    break;
                case ITEM_WAND:msg = "$p sparks and sputters!";
                    break;
                case ITEM_DEVICE:msg = "$p sparks and sputters!";
                    break;
    ...
    if (chance(ch, 2 * level) && !saves_breath(level, victim))
    {
        for (obj_lose = victim->first_carrying; obj_lose; obj_lose = obj_next)
        {
            ...
            switch (obj_lose->item_type)
            {
                ...
                case ITEM_CONTAINER:
                case ITEM_DRINK_CON:
                case ITEM_POTION:msg = "$p freezes and shatters!";
                    break;
            }
...
ch_ret spell_farsight(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    */
    if ((victim = get_char_world(ch, target_name)) == nullptr
        || victim == ch
        || !victim->in_room
...
ch_ret spell_recharge(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (obj->item_type == ITEM_STAFF || obj->item_type == ITEM_WAND)
    {
        ...
        {
            act(AT_FIRE, "$p bursts into flames, injuring you!", ch, obj, nullptr, TO_CHAR);
            act(AT_FIRE, "$p bursts into flames, charring $n!", ch, obj, nullptr, TO_ROOM);
            extract_obj(obj);
        ...
        if (chance(ch, 2))
        {
            act(AT_YELLOW, "$p glows with a blinding magical luminescence.", ch, obj, nullptr, TO_CHAR);
            obj->value[1] *= 2;
        ...
        {
            act(AT_YELLOW, "$p glows brightly for a few seconds...", ch, obj, nullptr, TO_CHAR);
            obj->value[2] = obj->value[1];
            return rNONE;
        }
        else if (chance(ch, 10))
        {
            act(AT_WHITE, "$p disintegrates into a void.", ch, obj, nullptr, TO_CHAR);
            act(AT_WHITE, "$n's attempt at recharging fails, and $p disintegrates.", ch, obj, nullptr, TO_ROOM);
            extract_obj(obj);
            return rSPELL_FAILED;
        }
        ...
        {
            act(AT_MAGIC, "$p feels warm to the touch.", ch, obj, nullptr, TO_CHAR);
            --obj->value[1];
            obj->value[2] = obj->value[1];
            return rNONE;
        }
...
ch_ret spell_remove_invis(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (obj)
    {
        ...
        act(AT_MAGIC, "$p becomes visible again.", ch, obj, nullptr, TO_CHAR);
...
ch_ret spell_animate_dead(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (get_mob_index(MOB_VNUM_ANIMATED_CORPSE) == nullptr)
    {
    ...
    if ((pMobIndex = get_mob_index((short)abs(corpse->cost))) == nullptr)
    {
...
ch_ret spell_scorching_surge(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(AT_MAGIC, "A fiery current lashes through $n's body!", ch, nullptr, nullptr, TO_ROOM);
    act(AT_MAGIC, "A fiery current lashes through your body!", ch, nullptr, nullptr, TO_CHAR);
...
bool check_save(int sn, int level, CHAR_DATA* ch, CHAR_DATA* victim)
{
    ...
    if (skill->saves)
        switch (skill->saves)
        {
            case SS_POISON_DEATH:saved = saves_poison_death(level, victim);
                break;
            case SS_ROD_WANDS:saved = saves_wands(level, victim);
                break;
            case SS_PARA_PETRI:saved = saves_para_petri(level, victim);
                break;
            case SS_BREATH:saved = saves_breath(level, victim);
                break;
            case SS_SPELL_STAFF:saved = saves_spell_staff(level, victim);
                break;
        }
    return saved;
}
       ...
       */
        switch (af.bitvector)
        {
            ...
            case AFF_POISON:af.type = gsn_poison;
                ...
                break;
            case AFF_BLIND:af.type = gsn_blindness;
                break;
            case AFF_INVISIBLE:af.type = gsn_invis;
                break;
            ...
            case AFF_CHARM:af.type = gsn_charm_person;
                ...
                break;
            case AFF_POSSESS:af.type = gsn_possess;
                break;
        }
        ...
        if (af.duration == 0)
        {
            ...
            switch (af.location)
            {
                ...
                case APPLY_MOVE:
                    ...
                    update_pos(victim);
                    break;
                default:affect_modify(victim, &af, TRUE);
                    break;
            }
        }
...
ch_ret spell_affect(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    for (; victim; victim = victim->next_in_room)
    {
        if (groupsp || areasp)
        {
            ...
            {
                act(AT_MAGIC, skill->hit_vict, ch, nullptr, victim, TO_VICT);
                if (hitroom)
                {
                    act(AT_MAGIC, skill->hit_room, ch, nullptr, victim, TO_NOTVICT);
                    act(AT_MAGIC, skill->hit_room, ch, nullptr, victim, TO_CHAR);
                }
            }
            else if (hitroom)
            ...
            {
                if (hitvict)
                    act(AT_MAGIC, skill->hit_vict, ch, nullptr, ch, TO_CHAR);
                else if (hitchar)
                    act(AT_MAGIC, skill->hit_char, ch, nullptr, ch, TO_CHAR);
            }
            else if (hitchar)
    ...
    switch (SPELL_ACTION(skill))
    {
        ...
        case SA_CREATE:
                ...
                || SPELL_CLASS(skill) == SC_DEATH)
            {
                switch (obj->item_type)
                {
                    default:failed_casting(skill, ch, nullptr, obj);
                        break;
                    case ITEM_FOOD:
                    case ITEM_DRINK_CON:separate_obj(obj);
                        obj->value[3] = 1;
                        successful_casting(skill, ch, nullptr, obj);
                        break;
                }
            ...
            {
                failed_casting(skill, ch, nullptr, obj);
                return rNONE;
            }
...
ch_ret spell_create_obj(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    switch (SPELL_POWER(skill))
    {
        ...
        case SP_MINOR:lvl = 0;
            break;
        case SP_GREATER:lvl = level / 2;
            break;
        case SP_MAJOR:lvl = level;
            break;
    }
    ...
    */
    ...
    if ((oi = get_obj_index(vnum)) == nullptr || (obj = create_object(oi, lvl)) == nullptr)
    {
        failed_casting(skill, ch, nullptr, nullptr);
        return rNONE;
    }
...
ch_ret spell_create_mob(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    */
    ...
    if ((mi = get_mob_index(vnum)) == nullptr || (mob = create_mobile(mi)) == nullptr)
    {
        failed_casting(skill, ch, nullptr, nullptr);
        return rNONE;
    }
    ...
    mob->max_hit = mob->top_level * 8 +
                   number_range(mob->top_level * mob->top_level / 4, mob->top_level * mob->top_level);
    mob->hit     = mob->max_hit;
    mob->gold    = 0;
...
ch_ret spell_smaug(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    switch (skill->target)
    {
        ...
        case TAR_OBJ_INV:return spell_obj_inv(sn, level, ch, vo);
    }
    return rNONE;
}
...
ch_ret spell_ethereal_fist(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(
        AT_MAGIC, "A fist of black, otherworldly ether rams into $N, leaving $M looking stunned!", ch, nullptr,
        victim, TO_NOTVICT
    );
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_spectral_furor(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(AT_MAGIC, "The fabric of the cosmos strains in fury about $N!", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_hand_of_chaos(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam = 0;
    act(AT_MAGIC, "$N is grasped by an incomprehensible hand of darkness!", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_disruption(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam = 0;
    act(
        AT_MAGIC, "A weird energy encompasses $N, causing you to question $S continued existence.", ch, nullptr,
        victim, TO_NOTVICT
    );
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_sonic_resonance(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam = dam * 3 / 4;
    act(AT_MAGIC, "A cylinder of kinetic energy enshrouds $N causing $S to resonate.", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_mind_wrack(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    */
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(AT_MAGIC, "$n stares intently at $N, causing $N to seem very lethargic.", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_mind_wrench(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    */
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(AT_MAGIC, "$n stares intently at $N, causing $N to seem very hyperactive.", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_revive(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    */
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(AT_MAGIC, "$n concentrates intently, and begins looking more centered.", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_sulfurous_spray(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam /= 4;
    act(AT_MAGIC, "A stinking spray of sulfurous liquid rains down on $N.", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_acetum_primus(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam = 3 * dam / 4;
    act(
        AT_MAGIC, "A cloak of primal acid enshrouds $N, sparks form as it consumes all it touches. ", ch, nullptr,
        victim, TO_NOTVICT
    );
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_magnetic_thrust(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(AT_MAGIC, "An unseen energy moves nearby, causing your hair to stand on end!", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_quantum_spike(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(
        AT_MAGIC, "$N seems to dissolve into tiny unconnected particles, then is painfully reassembled.", ch, nullptr,
        victim, TO_NOTVICT
    );
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_black_hand(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_poison_death(level, victim))
        dam /= 4;
    act(AT_MAGIC, "$n conjures a mystical hand, which swoops menacingly at $N.", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_black_fist(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_poison_death(level, victim))
        dam /= 4;
    act(AT_MAGIC, "$n forms a fist with the force, which swoops menacingly at $N.", ch, nullptr, victim, TO_NOTVICT);
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_black_lightning(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(
        AT_BLUE, "Bolts of electricity shoot from your fingertips, sending $N into a fit of painful spasms.", ch, nullptr,
        victim, TO_CHAR
    );
    act(
        AT_BLUE,
