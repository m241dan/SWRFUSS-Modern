...
ch_ret obj_cast_spell(int sn, int level, CHAR_DATA* ch, CHAR_DATA* victim, OBJ_DATA* obj)
{
    ...
    */
        ...
        && skill->type != SKILL_HERB && !chance(ch, 95 + levdiff))
    {
        switch (number_bits(2))
        {
            case 0:failed_casting(skill, ch, victim, NULL);
                break;
            ...
            case 2:failed_casting(skill, ch, victim, NULL);
                break;
    ...
    switch (SPELL_ACTION(skill))
    {
        ...
        case SA_CREATE:
                ...
                || SPELL_CLASS(skill) == SC_DEATH)
            {
                switch (obj->item_type)
                {
                    default:failed_casting(skill, ch, NULL, obj);
                        break;
                    case ITEM_FOOD:
...
ch_ret spell_ethereal_fist(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(
        AT_MAGIC, "A fist of black, otherworldly ether rams into $N, leaving $M looking stunned!", ch, NULL,
        victim, TO_NOTVICT
    );
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_disruption(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam = 0;
    act(
        AT_MAGIC, "A weird energy encompasses $N, causing you to question $S continued existence.", ch, NULL,
        victim, TO_NOTVICT
    );
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_acetum_primus(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam = 3 * dam / 4;
    act(
        AT_MAGIC, "A cloak of primal acid enshrouds $N, sparks form as it consumes all it touches. ", ch, NULL,
        victim, TO_NOTVICT
    );
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_quantum_spike(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    if (saves_spell_staff(level, victim))
        dam /= 2;
    act(
        AT_MAGIC, "$N seems to dissolve into tiny unconnected particles, then is painfully reassembled.", ch, NULL,
        victim, TO_NOTVICT
    );
    return damage(ch, victim, dam, sn);
}
...
ch_ret spell_black_lightning(int sn, int level, CHAR_DATA* ch, void* vo)
{
    ...
    act(
        AT_BLUE, "Bolts of electricity shoot from your fingertips, sending $N into a fit of painful spasms.", ch, NULL,
        victim, TO_CHAR
    );
    act(
        AT_BLUE,
