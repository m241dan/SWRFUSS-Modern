...
bool check_pos(CHAR_DATA* ch, short position)
{
    if (ch->position < position)
    {
        switch (ch->position)
        {
            case POS_DEAD:send_to_char("A little difficult to do when you are DEAD...\r\n", ch);
                break;
            ...
            case POS_SITTING:send_to_char("You can't do that sitting down.\r\n", ch);
                break;
...
void interpret(CHAR_DATA* ch, const char* argument)
{
    ...
    {
        snprintf(
            log_buf, MAX_STRING_LENGTH, "[*****] LAG: %s: %s %s (R:%d S:%d.%06d)", ch->name,
...
void send_timer(struct timerset* vtime, CHAR_DATA* ch)
{
    ...
    ch_printf(
        ch, "Time (in secs): min %ld.%06ld; avg: %ld.%06ld; max %ld.%06ld"
