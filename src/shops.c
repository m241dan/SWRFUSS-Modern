...
void do_buy(CHAR_DATA* ch, const char* argument)
{
    ...
    else
    {
        ...
        if (!IS_OBJ_STAT(obj, ITEM_INVENTORY) && (noi > 1))
        {
            ...
            act(
                AT_TELL, "$n tells you 'I don't have enough of those in stock"
    ...
    else
    {
        ...
        switch (obj->item_type)
        {
            default:send_to_char("For some reason, you think you got ripped off...\r\n", ch);
                break;
            case ITEM_ARMOR:obj->value[0] = obj->value[1];
                break;
            case ITEM_WEAPON:obj->value[0] = INIT_WEAPON_CONDITION;
                break;
            case ITEM_DEVICE:obj->value[2] = obj->value[1];
                break;
        }
...
void do_repair(CHAR_DATA* ch, const char* argument)
{
    ...
    {
        default:
        case SHOP_FIX:fixstr = "repair";
            fixstr2          = "repairs";
            break;
        case SHOP_RECHARGE:fixstr = "recharge";
            fixstr2               = "recharges";
            break;
    }
...
void do_shopstat(CHAR_DATA* ch, const char* argument)
{
    ...
    ch_printf(
        ch, "buy0 [%s]  buy1 [%s]  buy2 [%s]  buy3 [%s]  buy4 [%s]\r\n",
...
void do_shops(CHAR_DATA* ch, const char* argument)
{
    ...
    for (shop = first_shop; shop; shop = shop->next)
        ch_printf(
            ch, "Keeper: %5d Buy: %3d Sell: %3d Open: %2d Close: %2d Buy: %2d %2d %2d %2d %2d\r\n",
...
void do_repairshops(CHAR_DATA* ch, const char* argument)
{
    ...
    for (repair = first_repair; repair; repair = repair->next)
        ch_printf(
            ch, "Keeper: %5d Profit: %3d Type: %d Open: %2d Close: %2d Fix: %2d %2d %2d\r\n",
