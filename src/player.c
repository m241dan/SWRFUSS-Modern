...
void do_score(CHAR_DATA* ch, const char* argument)
{
    ...
    ch_printf(
        ch, "Race: %3d year old %-10.10s                Log In:  %s\r",
        get_age(ch), capitalize(get_race(ch)), ctime(&(ch->logon)));

    ch_printf(
        ch, "Hitroll: %-2.2d  Damroll: %-2.2d   Armor: %-4d        Saved:  %s\r",
    ...
    ch_printf(
        ch, "CREDITS: %-10d   BANK: %-10ld    Pkills: %-5.5d   Mkills: %-5.5d\r\n",
        ch->gold, ch->pcdata->bank, ch->pcdata->pkills, ch->pcdata->mkills
    );

    ch_printf(
        ch, "Weight: %5.5d (max %7.7d)    Items: %5.5d (max %5.5d)\r\n",
        ch->carry_weight, can_carry_w(ch), ch->carry_number, can_carry_n(ch));

    ch_printf(
        ch, "Pager: (%c) %3d   AutoExit(%c)  AutoLoot(%c)  Autosac(%c)\r\n",
    ...
    switch (ch->position)
    {
        case POS_DEAD:send_to_char("You are slowly decomposing. ", ch);
            break;
        case POS_MORTAL:send_to_char("You are mortally wounded. ", ch);
            break;
        case POS_INCAP:send_to_char("You are incapacitated. ", ch);
            break;
        case POS_STUNNED:send_to_char("You are stunned. ", ch);
            break;
        case POS_SLEEPING:send_to_char("You are sleeping. ", ch);
            break;
        case POS_RESTING:send_to_char("You are resting. ", ch);
            break;
        case POS_STANDING:send_to_char("You are standing. ", ch);
            break;
        case POS_FIGHTING:send_to_char("You are fighting. ", ch);
            break;
        case POS_MOUNTED:send_to_char("You are mounted. ", ch);
            break;
        case POS_SITTING:send_to_char("You are sitting. ", ch);
            break;
        default:send_to_char("Unknown ", ch);
    }
    ...
    if (ch->pcdata->clan)
    {
        send_to_char("----------------------------------------------------------------------------\r\n", ch);
        ch_printf(
            ch, "ORGANIZATION: %-35s Pkills/Deaths: %3.3d/%3.3d",
    ...
    if (IS_IMMORTAL(ch))
    {
        ...
        ch_printf(
            ch, "IMMORTAL DATA:  Wizinvis [%s]  Wizlevel (%d)\r\n",
        ...
        {
            ch_printf(
                ch, "Vnums:   Room (%-5.5d - %-5.5d)   Object (%-5.5d - %-5.5d)   Mob (%-5.5d - %-5.5d)\r\n",
...
void do_oldscore(CHAR_DATA* ch, const char* argument)
{
    ...
    switch (ch->position)
    {
        ...
        case POS_FIGHTING:send_to_char("You are fighting.\r\n", ch);
            break;
        case POS_MOUNTED:send_to_char("Mounted.\r\n", ch);
            break;
        case POS_SHOVE:send_to_char("Being shoved.\r\n", ch);
            break;
        case POS_DRAG:send_to_char("Being dragged.\r\n", ch);
            break;
    }
    ...
    {
        ch_printf(
            ch, "WizInvis level: %d   WizInvis is %s\r\n",
...
void do_description(CHAR_DATA* ch, const char* argument)
{
    ...
    switch (ch->substate)
    {
        default:bug("%s: illegal substate", __func__);
            return;

        case SUB_RESTRICTED:send_to_char("You cannot use this command from within another command.\r\n", ch);
            return;

        case SUB_NONE:ch->substate = SUB_PERSONAL_DESC;
            ch->dest_buf           = ch;
            start_editing(ch, ch->description);
            return;

        case SUB_PERSONAL_DESC:STRFREE(ch->description);
...
void do_bio(CHAR_DATA* ch, const char* argument)
{
    ...
    switch (ch->substate)
    {
        ...
        case SUB_NONE:ch->substate = SUB_PERSONAL_BIO;
            ch->dest_buf           = ch;
            start_editing(ch, ch->pcdata->bio);
            return;

        case SUB_PERSONAL_BIO:STRFREE(ch->pcdata->bio);
