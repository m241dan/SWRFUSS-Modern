...
void fread_clan(CLAN_DATA* clan, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (UPPER(word[0]))
        {
            ...
            case 'B':KEY("Board", clan->board, fread_number(fp));
                break;

            case 'D':KEY("Description", clan->description, fread_string(fp));
                break;
            ...
            case 'F':KEY("Funds", clan->funds, fread_number(fp));
                KEY("Filename", clan->filename, fread_string_nohash(fp));
                break;

            case 'G':KEY("GuardOne", clan->guard1, fread_number(fp));
                KEY("GuardTwo", clan->guard2, fread_number(fp));
                break;

            case 'J':KEY("Jail", clan->jail, fread_number(fp));
                break;

            case 'L':KEY("Leader", clan->leader, fread_string(fp));
                break;

            case 'M':KEY("MDeaths", clan->mdeaths, fread_number(fp));
                KEY("Members", clan->members, fread_number(fp));
                KEY("MKills", clan->mkills, fread_number(fp));
                KEY("MainClan", clan->tmpstr, fread_string(fp));
                break;

            case 'N':KEY("Name", clan->name, fread_string(fp));
                KEY("NumberOne", clan->number1, fread_string(fp));
                KEY("NumberTwo", clan->number2, fread_string(fp));
                break;

            case 'P':KEY("PDeaths", clan->pdeaths, fread_number(fp));
            ...
            case 'S':KEY("Storeroom", clan->storeroom, fread_number(fp));
                break;

            case 'T':KEY("Type", clan->clan_type, fread_number(fp));
...
void fread_planet(PLANET_DATA* planet, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (UPPER(word[0]))
        {
            ...
            case 'B':KEY("BaseValue", planet->base_value, fread_number(fp));
                break;
            ...
            case 'F':KEY("Filename", planet->filename, fread_string_nohash(fp));
            ...
            case 'N':KEY("Name", planet->name, fread_string(fp));
                break;

            case 'P':KEY("PopSupport", planet->pop_support, fread_float(fp));
                break;
            ...
            case 'T':KEY("Taxes", planet->base_value, fread_number(fp));
                break;
...
void do_clans(CHAR_DATA* ch, const char* argument)
{
    ...
    for (clan = first_clan; clan; clan = clan->next)
    {
        ...
        ch_printf(
            ch, "  &WPlanets: &O%-2d       &WAvg Pop Support: &O%-3d&W    &WRevenue: &O%-10ld\r\n",
            pCount, support, revenue
        );
        ch_printf(ch, "  &WDivision             Leaders             Spacecraft/Vehicles/Members/Funds\r\n");
        ch_printf(
            ch, "  &O%-20s %-10s %-10s %-10s %-2d %-2d %-3d %ld\r\n",
    ...
    for (clan = first_clan; clan; clan = clan->next)
    {
        ...
        ch_printf(
            ch, "&Y%-22s &O%-10s %-10s %-10s %-2d %-2d %-3d %ld\r\n",
...
void do_planets(CHAR_DATA* ch, const char* argument)
{
    ...
    {
        ch_printf(
            ch, "&WPlanet: &G%-15s   &WGoverned By: &G%s %s\r\n",
            planet->name,
