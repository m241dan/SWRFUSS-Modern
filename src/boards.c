...
void do_noteroom(CHAR_DATA* ch, const char* argument)
{
    ...
    switch (ch->substate)
    {
        case SUB_WRITING_NOTE:do_note(ch, arg_passed, FALSE);
            break;
...
void do_mailroom(CHAR_DATA* ch, const char* argument)
{
    ...
    switch (ch->substate)
    {
        case SUB_WRITING_NOTE:do_note(ch, arg_passed, TRUE);
            break;
...
BOARD_DATA* read_board(const char* boardfile, FILE* fp)
{
    ...
    for (;;)
    {
        ...
        switch (UPPER(word[0]))
        {
            ...
            case 'F':KEY("Filename", board->note_file, fread_string_nohash(fp));
            case 'M':KEY("Min_read_level", board->min_read_level, fread_number(fp));
            ...
            case 'P':KEY("Post_group", board->post_group, fread_string_nohash(fp));
            case 'R':KEY("Read_group", board->read_group, fread_string_nohash(fp));
            case 'T':KEY("Type", board->type, fread_number(fp));
            case 'V':KEY("Vnum", board->board_obj, fread_number(fp));
        }
        if (!fMatch)
        {
...
void do_bstat(CHAR_DATA* ch, const char* argument)
{
    ...
    ch_printf(
        ch, "%-12s Vnum: %5d Read: %2d Post: %2d Rmv: %2d Max: %2d Posts: %d Type: %d\r\n",
    ...
    ch_printf(
        ch, "Read_group: %-15s Post_group: %-15s \r\nExtra_readers: %-10s\r\n",
...
void do_boards(CHAR_DATA* ch, const char* argument)
{
    ...
    for (board = first_board; board; board = board->next)
        ch_printf(
            ch, "%-16s Vnum: %5d Read: %2d Post: %2d Rmv: %2d Max: %2d Posts: %d Type: %d\r\n",
            board->note_file, board->board_obj,
            board->min_read_level, board->min_post_level,
            board->min_remove_level, board->max_posts, board->num_posts, board->type
        );
